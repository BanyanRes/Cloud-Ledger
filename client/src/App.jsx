  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><div><div style={S.h1}>Chart of Accounts</div><div style={S.sub}>{accounts.length} accounts</div></div>
    ...
    {editing&&<div style={{...S.card,borderColor:T.accent+'40',marginBottom:16}}>
      ...
      <div style={S.row}>
        ...
        <div style={S.col}><label style={S.label}>Type</label><select style={S.select} value={editForm.type} onChange={e=>setEditForm(f=>({...f,type:e.target.value}))}>{['Asset','Liability','Equity','Revenue','Expense'].map(t=><option key={t}>{t}</option>)}</select></div>
        <div style={S.col}><label style={S.label}>Subtype</label><input style={S.input} value={editForm.subtype} onChange={e=>setEditForm(f=>({...f,subtype:e.target.value}))}/></div></div>
    ...
    <div style={S.cardFlush}><table style={S.table}><thead><tr><th style={S.th}>Code</th><th style={S.th}>Name</th><th style={S.th}>Type</th><th style={S.th}>Subtype</th><th style={S.thC}>Bank</th>{canEdit&&<th style={{...S.th,width:80}}>Actions</th>}</tr></thead>
      <tbody>{accounts.map(a=><tr key={a.code} style={editing===a.code?{background:T.accentDim}:{}}>
        <td style={{...S.td,fontWeight:600,color:T.textBright}}>{a.code}</td><td style={S.td}>{a.name}</td><td style={S.td}><span style={S.tag(a.type)}>{a.type}</span></td><td style={{...S.td,color:T.textDim}}>{a.subtype}</td>
