...
import { api } from './api';
import logoSrc from './cloudledger-logo.png';
...
function Logo({size=32, withWordmark=false}) {
  return (
    <div style={{display:'flex',alignItems:'center',gap:withWordmark?10:0}}>
      <img src={logoSrc} alt="Cloud Ledger" style={{height:size,width:'auto',display:'block'}}/>
      {withWordmark && <div style={{fontSize:18,fontWeight:800,color:T.textBright,letterSpacing:'-0.02em'}}>CloudLedger</div>}
    </div>
  );
}

const FILE_ACCEPT = '.pdf,.xlsx,.xls,.csv,.png,.jpg,.jpeg,.eml,.msg,.doc,.docx';
const FILE_EXT_OK = ['pdf','xlsx','xls','csv','png','jpg','jpeg','eml','msg','doc','docx'];
const fileExt = name => (String(name||'').split('.').pop() || '').toLowerCase();
const isPreviewableAttachment = att => {
  const ext = fileExt(att?.original_name || att?.name);
  return ['pdf','png','jpg','jpeg'].includes(ext) || ['application/pdf','image/png','image/jpeg','image/jpg'].includes(att?.mime_type);
};
const normalizeText = s => String(s || '').toLowerCase().replace(/\b\d{3,}\b/g, ' ').replace(/[^a-z]+/g, ' ').replace(/\s+/g, ' ').trim();
const normalizeVendor = s => normalizeText(s)
  .replace(/\b(card|purchase|debit|credit|ach|pos|sq|check|transfer|online|payment|withdrawal|deposit|visa|mc|txn|trace|id)\b/g, ' ')
  .replace(/\s+/g, ' ')
  .trim();
const smartContains = (hay, needles=[]) => needles.some(n => hay.includes(n));

const HINTS = [
  { keys:['amazon','office depot','staples','fedex office','ups store'], subtype:['Operating Expense'], match:/office|suppl|amazon/i },
  { keys:['google workspace','google','microsoft','adobe','dropbox','slack','zoom','notion','openai','anthropic'], subtype:['Operating Expense'], match:/software|subscription|office|marketing/i },
  { keys:['gusto','adp','paychex','payroll','salary','wage','bonus'], subtype:['Operating Expense'], match:/salary|payroll/i },
  { keys:['rent','lease','landlord','property mgmt'], subtype:['Operating Expense'], match:/rent/i },
  { keys:['att','verizon','tmobile','comcast','spectrum','internet','phone'], subtype:['Operating Expense'], match:/tele|internet|utilit|office/i },
  { keys:['electric','gas','water','utility','utilities','edison'], subtype:['Operating Expense'], match:/utilit/i },
  { keys:['state farm','farmers','insurance','premium','liberty mutual'], subtype:['Operating Expense'], match:/insurance/i },
  { keys:['facebook','meta','google ads','linkedin','advertising','marketing'], subtype:['Operating Expense'], match:/market/i },
  { keys:['interest','finance charge','loan payment'], subtype:['Other Expense'], match:/interest/i },
  { keys:['stripe','shopify','square','wire in','ach credit','payment received','deposit','revenue','sales'], subtype:['Operating Revenue','Other Revenue'], match:/revenue|income/i },
];

function suggestAccount(desc, accounts, bankCode, history=[]) {
  const vendor = normalizeVendor(desc);
  const usable = accounts.filter(a => a.code !== bankCode);
  if (!usable.length || !vendor) return null;

  const historyHits = history.filter(t => t.account_code && normalizeVendor(t.description) === vendor);
  if (historyHits.length) {
    const byCount = historyHits.reduce((m, t) => {
      m[t.account_code] = (m[t.account_code] || 0) + 1;
      return m;
    }, {});
    const [best] = Object.entries(byCount).sort((a,b) => b[1]-a[1]);
    const acct = usable.find(a => a.code === best?.[0]);
    if (acct) return acct;
  }

  for (const h of HINTS) {
    if (!smartContains(vendor, h.keys)) continue;
    const choices = usable.filter(a => h.subtype.includes(a.subtype));
    const exact = choices.find(a => h.match.test(a.name));
    if (exact) return exact;
    if (choices[0]) return choices[0];
  }

  if (/owner draw|distribution|member draw/.test(vendor)) {
    return usable.find(a => a.type === 'Equity') || null;
  }
  return null;
}

function AttachmentChip({attachment,onPreview,showPreview=true}){
  const previewable = isPreviewableAttachment(attachment);
  const url = api.downloadAttachment(attachment.id);
  return <span style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 10px',borderRadius:999,border:'1px solid '+T.border,background:'#fff',fontSize:12,boxShadow:'0 1px 2px rgba(15,23,42,0.04)'}}>
    <span style={{fontWeight:600,color:T.textMuted,textTransform:'uppercase'}}>{fileExt(attachment.original_name || attachment.name) || 'file'}</span>
    <a href={url} target="_blank" rel="noreferrer" style={{color:T.textBright,textDecoration:'none',maxWidth:220,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{attachment.original_name || attachment.name}</a>
    {showPreview && previewable && <button style={{...S.btnGhost,color:T.accent,padding:'0 4px',fontWeight:600}} onClick={()=>onPreview?.(attachment)}>Preview</button>}
  </span>;
}
...
function AttachmentPreviewModal({attachment,onClose}){
  ...
  const url = api.downloadAttachment(attachment.id);
  return <div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:1100,height:'88vh',padding:20,display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
    <button style={S.modalClose} onClick={onClose}>&times;</button>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingRight:40,marginBottom:14}}>
      <div style={{fontSize:16,fontWeight:700,color:T.textBright,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{attachment.original_name}</div>
      <a href={url} target="_blank" rel="noreferrer" style={{...S.btnS,textDecoration:'none'}}>Open file</a>
    </div>
    <div style={{flex:1,border:'1px solid '+T.border,borderRadius:14,overflow:'hidden',background:'#fff'}}>
      {isPreviewableAttachment(attachment)
        ? <iframe title={attachment.original_name} src={url} style={{width:'100%',height:'100%',border:'none'}} />
        : <div style={{height:'100%',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12,color:T.textMuted}}><div>No inline preview for this file type.</div><a href={url} target="_blank" rel="noreferrer" style={{...S.btnP,textDecoration:'none'}}>Open attachment</a></div>}
    </div>
  </div></div>;
}
...
const T = {
  bg: '#f5f7fb', bgCard: '#ffffff', bgHover: '#f8fafc', bgElevated: '#f8fafc',
  border: '#e2e8f0', borderLight: '#eef2f7', borderFocus: '#3b82f640',
  text: '#334155', textBright: '#0f172a', textMuted: '#64748b', textDim: '#94a3b8',
  accent: '#2563eb', accentLight: '#3b82f6', accentDim: '#2563eb12',
  green: '#059669', greenDim: '#05966912', greenBorder: '#05966930',
  red: '#dc2626', redDim: '#dc262610', orange: '#d97706', orangeDim: '#d9770610',
  purple: '#7c3aed', purpleDim: '#7c3aed10', teal: '#0f766e', tealDim: '#0f766e10',
  shadow: '0 10px 30px rgba(15,23,42,0.06)',
  shadowLg: '0 18px 48px rgba(15,23,42,0.14)',
  radius: '16px', radiusSm: '12px', radiusXs: '10px',
  sidebarBg: '#0f172a', sidebarText: '#94a3b8', sidebarActive: '#f8fafc', sidebarAccent: '#60a5fa',
};
...
const S = {
  ...
  topBar: { background: 'rgba(255,255,255,0.92)', backdropFilter:'blur(10px)', borderBottom: '1px solid '+T.border, padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, zIndex: 10, boxShadow: T.shadow },
  ...
  main: { flex: 1, padding: '32px 36px', overflowY: 'auto', background: T.bg },
  ...
  btnP: { background: 'linear-gradient(135deg,'+T.accent+','+T.accentLight+')', color: '#fff', border: 'none', borderRadius: T.radiusXs, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow:'0 10px 22px rgba(37,99,235,0.18)' },
  btnS: { background: '#fff', color: T.text, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '10px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
...
function AccountAutocomplete({accounts,value,onChange,placeholder,exclude}){
  ...
  const filtered=useMemo(()=>{const s=q.toLowerCase();return accounts.filter(a=>(!exclude||a.code!==exclude)&&(a.code.toLowerCase().includes(s)||a.name.toLowerCase().includes(s))).slice(0,12);},[accounts,q,exclude]);
  ...
  return(<div ref={ref} style={{position:'relative'}}><input style={S.inputSm} placeholder={placeholder||'Search account...'} value={open?q:(sel?acctLabel(sel.code,sel.name):'')}
    ...
    {open&&filtered.length>0&&<div style={{position:'absolute',top:'100%',left:0,right:0,background:'#fff',border:'1px solid '+T.border,borderRadius:T.radiusSm,maxHeight:220,overflowY:'auto',zIndex:50,boxShadow:T.shadowLg,marginTop:4}}>
...
function AuthScreen({onLogin}){const[mode,setMode]=useState('login');const[email,setEmail]=useState('');const[pw,setPw]=useState('');const[name,setName]=useState('');const[confirmPw,setConfirmPw]=useState('');const[role,setRole]=useState('Accountant');
  ...
  return(<div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:'#f1f5f9'}}>
    <div style={{background:'#fff',border:'1px solid '+T.border,borderRadius:16,width:420,padding:44,textAlign:'center',boxShadow:T.shadowLg}}>
      <div style={{margin:'0 auto 18px',display:'flex',justifyContent:'center'}}><Logo size={64}/></div>
...
function ChangePasswordModal({onClose}){const[cur,setCur]=useState('');const[nw,setNw]=useState('');const[cf,setCf]=useState('');const[err,setErr]=useState('');const[ok,setOk]=useState(false);
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:400,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
    ...
    {err&&<div style={S.err}>{err}</div>}{ok&&<div style={S.success}>Updated!</div>}
    <button style={{...S.btnP,width:'100%',padding:11,marginTop:8}} onClick={async()=>{if(nw.length<3){setErr('Min 3');return;}if(nw!==cf){setErr("Don't match");return;}try{await api.changePassword(cur,nw);setOk(true);setTimeout(onClose,1500);}catch(e){setErr(e.message);}}}>Update</button>
  </div></div>);}
...
function JournalEntryModal({entityId,user,onClose,onPosted,form,setForm,pendingFiles,setPendingFiles}){
  const[accounts,setAccounts]=useState([]);const[showAddAcct,setShowAddAcct]=useState(false);const[err,setErr]=useState('');const[posting,setPosting]=useState(false);const[posted,setPosted]=useState('');const[uploadMsg,setUploadMsg]=useState('');
  ...
  const discard=()=>{setForm(BLANK_JE());setPendingFiles([]);setErr('');setUploadMsg('');};
  const addPendingFiles=list=>{
    const incoming=Array.from(list||[]);
    const invalid=incoming.filter(f=>!FILE_EXT_OK.includes(fileExt(f.name)));
    if(invalid.length) setErr('Unsupported file type: '+invalid.map(f=>'.'+fileExt(f.name)).join(', '));
    const allowed=incoming.filter(f=>FILE_EXT_OK.includes(fileExt(f.name)));
    if(!allowed.length) return;
    setPendingFiles(p=>[...p,...allowed].slice(0,10));
    setUploadMsg(allowed.length+' support file'+(allowed.length>1?'s':'')+' ready to upload');
  };
  const post=async()=>{
    if(!form.date||!form.memo.trim()){setErr('Date and memo required');return;}
    if(form.lines.some(l=>!l.account_code)){setErr('All lines need an account');return;}
    if(!bal){setErr('Entry must balance');return;}
    setPosting(true);setErr('');setUploadMsg('');
    try{
      const r=await api.createEntry(entityId,{date:form.date,memo:form.memo.trim(),lines:form.lines.map(l=>({account_code:l.account_code,debit:parseFloat(l.debit)||0,credit:parseFloat(l.credit)||0}))});
      let msg='JE-'+String(r.entry_num).padStart(4,'0')+' posted';
      if(pendingFiles.length>0){
        const uploaded=await api.uploadAttachments(entityId,r.id,pendingFiles);
        msg+=' with '+uploaded.length+' attachment'+(uploaded.length>1?'s':'');
      }
      setForm(BLANK_JE());setPendingFiles([]);setPosted(msg+'!');setTimeout(()=>setPosted(''),5000);if(onPosted)onPosted();
    }catch(e){setErr(e.message || 'Unable to post entry');}finally{setPosting(false);}
  };
  const hasContent=form.memo||form.lines.some(l=>l.account_code||l.debit||l.credit)||pendingFiles.length>0;

  return(<div style={S.modal}><div style={{...S.modalBox,maxWidth:1040}} onClick={e=>e.stopPropagation()}>
    <button style={S.modalClose} onClick={onClose}>&times;</button>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
      <div>
        <div style={{fontSize:19,fontWeight:800,color:T.textBright}}>New Journal Entry</div>
        <div style={{fontSize:12,color:T.textMuted,marginTop:4}}>Add entry lines and support documents in one place.</div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        {hasContent&&<span style={{fontSize:11,color:T.orange,fontWeight:600,background:T.orangeDim,padding:'4px 10px',borderRadius:999}}>In progress</span>}
        {hasContent&&<button style={{...S.btnGhost,color:T.red,fontSize:12}} onClick={discard}>Discard</button>}
      </div></div>
    <div style={{background:T.bgElevated,border:'1px solid '+T.border,borderRadius:T.radius,padding:18,marginBottom:16}}>
    ...
    <div style={{...S.cardFlush,marginBottom:16}}><table style={S.table}><thead><tr><th style={S.th}>Account</th><th style={{...S.thR,width:140}}>Debit</th><th style={{...S.thR,width:140}}>Credit</th><th style={{...S.th,width:36}}></th></tr></thead>
      <tbody>{form.lines.map((l,i)=><tr key={i}><td style={{padding:'8px 10px',borderBottom:'1px solid '+T.borderLight}}>
        ...
        <td style={{padding:'8px 10px',borderBottom:'1px solid '+T.borderLight}}><input style={{...S.input,textAlign:'right'}} placeholder="0.00" value={l.debit} onChange={e=>updateLine(i,'debit',e.target.value)}/></td>
        <td style={{padding:'8px 10px',borderBottom:'1px solid '+T.borderLight}}><input style={{...S.input,textAlign:'right'}} placeholder="0.00" value={l.credit} onChange={e=>updateLine(i,'credit',e.target.value)}/></td>
    ...
    <div style={{border:'1px solid '+(pendingFiles.length>0?T.teal+'40':T.border),borderRadius:T.radius,padding:18,marginBottom:16,background:pendingFiles.length>0?T.tealDim:'#fff'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:pendingFiles.length>0?12:0,gap:12,flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:13,fontWeight:700,color:T.textBright}}>Support Files</div>
          <div style={{fontSize:12,color:T.textMuted,marginTop:4}}>Attach PDF, Excel, or email files so backup is saved with the entry.</div>
        </div>
        <div><input type="file" multiple accept={FILE_ACCEPT} style={{display:'none'}} id="je-file-input" onChange={e=>{addPendingFiles(e.target.files);e.target.value='';}}/><label htmlFor="je-file-input" style={{...S.btnS,cursor:'pointer',display:'inline-block'}}>Attach files</label></div></div>
      {pendingFiles.length>0
        ? <div style={{display:'flex',flexWrap:'wrap',gap:8}}>{pendingFiles.map((f,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:6,background:'#fff',padding:'8px 12px',borderRadius:999,fontSize:12,border:'1px solid '+T.border}}>
            <span style={{fontWeight:700,color:T.textMuted,textTransform:'uppercase'}}>{fileExt(f.name)}</span>
            <span style={{fontWeight:600,maxWidth:240,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.name}</span><span style={{color:T.textDim}}>({fmtSize(f.size)})</span>
        ...
        : <div style={{fontSize:12,color:T.textMuted}}>No files attached yet.</div>}
      {uploadMsg&&<div style={{...S.success,marginTop:10}}>{uploadMsg}</div>}
    </div>
    <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
      ...
      {bal&&<span style={{fontSize:12,color:T.green,fontWeight:700}}>Balanced</span>}
...
export default function App(){
  ...
  return(<div style={S.app}>
    <div style={S.topBar}><div style={{display:'flex',alignItems:'center',gap:16}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12}}><Logo size={36}/>{!sidebarCol&&<div style={{fontSize:18,fontWeight:800,color:T.textBright,letterSpacing:'-0.02em'}}>CloudLedger</div>}</div>
...
function JournalList({entityId,onNewEntry}){const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);const[from,setFrom]=useState('');const[to,setTo]=useState('');
  const[editEntry,setEditEntry]=useState(null);const[previewAttachment,setPreviewAttachment]=useState(null);
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><div><div style={S.h1}>Journal Entries</div><div style={S.sub}>{entries.length} entries</div></div><button style={S.btnP} onClick={onNewEntry}>+ New Entry</button></div>
    ...
    {entries.length===0?<div style={{...S.card,textAlign:'center',padding:60,color:T.textDim}}>No entries found</div>:
      entries.map(e=><div key={e.id} style={{...S.card,padding:18}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,gap:12,flexWrap:'wrap'}}>
          <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}><span style={{fontWeight:700,color:T.accent,fontSize:14}}>JE-{String(e.entry_num).padStart(4,'0')}</span>
            <span style={{color:T.textMuted}}>{e.date}</span><span style={{fontWeight:600}}>{e.memo}</span>
            {e.attachments?.length>0&&<span style={{fontSize:11,color:T.teal,fontWeight:700,background:T.tealDim,padding:'4px 10px',borderRadius:999}}>{e.attachments.length} attachment{e.attachments.length>1?'s':''}</span>}</div>
        ...
        {e.attachments?.length>0&&<div style={{marginTop:12,display:'flex',flexWrap:'wrap',gap:8}}>{e.attachments.map(a=><AttachmentChip key={a.id} attachment={a} onPreview={setPreviewAttachment}/>)}</div>}
      </div>)}
...
function BankTransactions({entityId}){const[accounts,setAccounts]=useState([]);const[bankAccts,setBankAccts]=useState([]);const[txns,setTxns]=useState([]);
  const[selAcct,setSelAcct]=useState('');const[statusFilter,setStatusFilter]=useState('');const[err,setErr]=useState('');const[msg,setMsg]=useState('');const[showAddAcct,setShowAddAcct]=useState(false);
  const[uploading,setUploading]=useState(false);const[uploadProgress,setUploadProgress]=useState('');const[lastBatch,setLastBatch]=useState(null);const[deletingBatch,setDeletingBatch]=useState(false);
  ...
  useEffect(()=>{load();},[load]);
  useEffect(()=>{setLastBatch(null);setMsg('');setErr('');},[selAcct]);
  const handleUpload=async e=>{const file=e.target.files[0];if(!file||!selAcct)return;e.target.value='';setErr('');setMsg('');setUploading(true);setUploadProgress('Uploading file...');
    try{const r=await api.uploadBankTransactions(entityId,selAcct,file);
      setLastBatch({id:r.batch_id,count:r.count,fileName:file.name});
      setUploadProgress('Auto-categorizing '+r.count+' transactions...');
      const [imported,history]=await Promise.all([api.getBankTransactions(entityId,selAcct,'pending'),api.getBankTransactions(entityId,selAcct)]);
      let auto=0;
      for(const t of imported.filter(x=>x.batch_id===r.batch_id)){if(!t.account_code){const sg=suggestAccount(t.description,accounts,selAcct,history.filter(h=>h.id!==t.id));if(sg){await api.codeBankTransaction(entityId,t.id,sg.code,t.memo||t.description);auto++;}}}
      setMsg(r.count+' imported'+(auto>0?', '+auto+' auto-categorized':''));
      load();}catch(ex){setErr(ex.message);}finally{setUploading(false);setUploadProgress('');}};
  const cancelUpload=()=>{setUploading(false);setUploadProgress('');setMsg('Upload cancelled before import');};
  const deleteBatch=async()=>{if(!lastBatch?.id)return;if(!confirm('Delete the uploaded bank transaction batch?'))return;setDeletingBatch(true);setErr('');
    try{const r=await api.deleteBankBatch(entityId,lastBatch.id);setMsg((r.deleted||0)+' uploaded transaction'+((r.deleted||0)!==1?'s':'')+' removed');setLastBatch(null);load();}catch(ex){setErr(ex.message);}finally{setDeletingBatch(false);}};
  ...
  const postCoded=async()=>{const ids=txns.filter(t=>t.status==='coded').map(t=>t.id);if(!ids.length){setErr('Nothing coded');return;}try{const r=await api.postBankTransactions(entityId,ids);setMsg(r.posted+' JEs created');setLastBatch(null);load();}catch(ex){setErr(ex.message);}};
  const totalIn=txns.filter(t=>t.amount>0).reduce((s,t)=>s+t.amount,0);const totalOut=txns.filter(t=>t.amount<0).reduce((s,t)=>s+Math.abs(t.amount),0);const uncat=txns.filter(t=>t.status==='pending').length;
  ...
  return(<div><div style={S.h1}>Bank Transactions</div><div style={S.sub}>Upload, categorize, and post bank activity to the general ledger</div>
    <div style={S.card}><div style={S.row}>
      ...
      {selAcct&&<div style={{display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}>
        {uploading?<><span style={{fontSize:12,color:T.accent,fontWeight:500,marginBottom:8}}>{uploadProgress}</span>
          <button style={{...S.btnD,padding:'9px 20px',fontSize:13}} onClick={cancelUpload}>Cancel</button></>
    ...
    {lastBatch&&<div style={{marginTop:14,padding:'14px 16px',border:'1px solid '+T.orange+'30',borderRadius:T.radiusSm,background:T.orangeDim,display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}>
      <div><div style={{fontWeight:700,color:T.textBright}}>Latest upload ready for review</div><div style={{fontSize:12,color:T.textMuted,marginTop:4}}>{lastBatch.count} transaction{lastBatch.count!==1?'s':''} imported from {lastBatch.fileName}. Use Delete Upload if you do not want to code this batch.</div></div>
      <button style={{...S.btnD,padding:'9px 18px',fontSize:13,opacity:deletingBatch?.6:1}} disabled={deletingBatch} onClick={deleteBatch}>{deletingBatch?'Deleting...':'Delete Upload'}</button>
    </div>}
    {err&&<div style={S.err}>{err}</div>}{msg&&<div style={S.success}>{msg}</div>}</div>
    ...
    {selAcct&&txns.length>0&&<div style={S.cardFlush}>
      <div style={{padding:'18px 20px',display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+T.border,flexWrap:'wrap',gap:10}}>
        <div style={S.h2}>{txns.length} Transactions</div>
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          <button style={{...S.btnS,color:T.teal,borderColor:T.teal+'40'}} onClick={()=>setShowAddAcct(true)}>+ New Account</button>
          {lastBatch&&<button style={{...S.btnD,padding:'9px 14px'}} onClick={deleteBatch}>Delete Upload</button>}
      ...
      <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>Description</th><th style={S.thR}>Amount</th><th style={{...S.th,width:260}}>GL Account</th><th style={{...S.th,width:170}}>Memo</th><th style={{...S.th,width:70}}>Status</th><th style={{...S.th,width:36}}></th></tr></thead>
        <tbody>{txns.map(t=><tr key={t.id} style={t.status==='posted'?{opacity:0.45}:{}}>
          ...
          <td style={{...S.td,maxWidth:260,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontWeight:600}}>{t.description}</td>
          ...
          <td style={S.td}><span style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:20,background:t.status==='posted'?T.greenDim:t.status==='coded'?T.accentDim:T.orangeDim,color:t.status==='posted'?T.green:t.status==='coded'?T.accent:T.orange}}>{t.status}</span></td>
    ...
    {selAcct&&txns.length===0&&<div style={{...S.card,textAlign:'center',padding:60,color:T.textDim}}>No transactions yet. Upload a bank statement above.</div>}
...
// ═══ Reports ═══
function TrialBalance({entityId}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const fyS=asOf.slice(0,4)+'-01-01';
...
function BalanceSheet({entityId}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const fyS=asOf.slice(0,4)+'-01-01';
  useEffect(()=>{api.getBalances(entityId,{as_of:asOf,close_pl_before:fyS}).then(setBalances);},[entityId,asOf,fyS]);
...
function UserManagement({currentUser}){
  ...
  const loadUsers=useCallback(()=>{api.getUsers().then(setUsers);},[]);
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>User Management</div><div style={S.sub}>{users.length} users</div></div>
    ...
    {showAdd&&<div style={{...S.card,borderColor:T.green+'40'}}><div style={S.row}>
      ...
      {err&&<div style={S.err}>{err}</div>}<button style={S.btnP} onClick={async()=>{if(!form.name||!form.email||!form.password){setErr('All fields required');return;}try{await api.signup(form.name,form.email,form.password,form.role);setForm({name:'',email:'',password:'',role:'Viewer'});setShowAdd(false);setErr('');loadUsers();}catch(e){setErr(e.message);}}}>Add User</button></div>}
    <div style={S.cardFlush}><table style={S.table}><thead><tr><th style={S.th}>Name</th><th style={S.th}>Email</th><th style={S.th}>Role</th><th style={{...S.th,width:240}}>Actions</th></tr></thead>
      <tbody>{users.map(u=><tr key={u.id}>
        <td style={{...S.td,fontWeight:600,color:T.textBright}}>{u.name}{u.id===currentUser.id?<span style={{color:T.accent,fontSize:10,marginLeft:8}}>(you)</span>:''}</td>
        ...
        <td style={S.td}><div style={{display:'flex',gap:8}}>
          ...
          {u.id!==currentUser.id&&<button style={{...S.btnD,padding:'5px 12px',fontSize:11}} onClick={async()=>{await api.deleteUser(u.id);loadUsers();}}>Delete</button>}</div></td>
      </tr>)}</tbody></table></div>
    {resetId&&<div style={S.modal} onClick={()=>setResetId(null)}><div style={{...S.modalBox,maxWidth:400,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{fontSize:13,color:T.textMuted,marginBottom:16}}>For: {users.find(u=>u.id===resetId)?.name} ({users.find(u=>u.id===resetId)?.email})</div>
