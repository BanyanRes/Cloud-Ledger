...
function AccountDrillDownModal({entityId,entityName,acct,from,to,onClose}){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try{
        // Pull all entries in the 12-month window
        const entries=await api.getEntries(entityId,from,to);
        const txns=[];
        ...
        // Beginning balance = current balance - (debits-credits) within window, normalized to natural side
           ...
           {lines.length===0?
             ...
             :lines.map((l,i)=>{running+=isDr?(l.debit-l.credit):(l.credit-l.debit);
               ...
               return<tr key={i}>
                 ...
                 <td style={S.td} title={tip}><span style={{color:T.accent,fontWeight:600}}>JE-{String(l.entry_num).padStart(4,'0')}</span></td>
