...
function JournalEntryModal({entityId,user,onClose,onPosted,jeForm,setJeForm,pendingFiles,setPendingFiles}){
  ...
  const post=async()=>{if(!jeForm.date||!jeForm.memo.trim()){setErr('Date & memo required');return;}if(jeForm.lines.some(l=>!l.account_code)){setErr('All lines need account');return;}if(!bal){setErr('Must balance');return;}
    ...
    try{
      const r=await api.createEntry(entityId,{date:jeForm.date,memo:jeForm.memo.trim(),lines:jeForm.lines.map(l=>({account_code:l.account_code,debit:parseFloat(l.debit)||0,credit:parseFloat(l.credit)||0}))});
      let attachMsg = '';
      if(pendingFiles.length>0){
        try { const uploaded = await api.uploadAttachments(entityId,r.id,pendingFiles); attachMsg = ' with '+uploaded.length+' attachment(s)'; }
        catch(uploadErr) { attachMsg = ' (entry posted but attachment upload failed: '+uploadErr.message+')'; }
      }
      setJeForm(BLANK_JE());setPendingFiles([]);setPosted('JE-'+String(r.entry_num).padStart(4,'0')+' posted'+attachMsg+'!');setTimeout(()=>setPosted(''),5000);if(onPosted)onPosted();
  ...
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:960}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{marginBottom:12,border:'1px solid '+(pendingFiles.length>0?C.teal+'50':C.border),borderRadius:8,padding:14,background:pendingFiles.length>0?C.teal+'08':'transparent'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:pendingFiles.length>0?10:0}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <span style={{fontSize:12,fontWeight:600,color:pendingFiles.length>0?C.teal:C.textDim}}>
            {pendingFiles.length>0?pendingFiles.length+' file(s) attached':'No attachments'}
          </span>
        </div>
        <div>
          <input type="file" multiple accept=".pdf,.xlsx,.xls,.csv,.png,.jpg,.jpeg,.eml,.msg,.doc,.docx" ref={fileInputRef} style={{display:'none'}} id="je-file-input" onChange={handleFileSelect}/>
          <label htmlFor="je-file-input" style={{...S.btnS,display:'inline-block',padding:'5px 14px',fontSize:11,cursor:'pointer',color:C.teal,borderColor:C.teal+'40'}}>+ Attach Files</label>
        </div>
      </div>
      {pendingFiles.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:4}}>
        {pendingFiles.map((f,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:6,background:C.border,padding:'5px 10px',borderRadius:5,fontSize:11}}>
          <span style={{color:C.textBright,fontWeight:500}}>{f.name}</span>
          <span style={{color:C.textDim}}>({fmtSize(f.size)})</span>
          <span style={{cursor:'pointer',color:C.red,fontWeight:700,marginLeft:2}} onClick={()=>setPendingFiles(p=>p.filter((_,j)=>j!==i))}>x</span>
        </div>)}
      </div>}
    </div>
    <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
      ...
      <button style={{...S.btnP,opacity:posting?.6:1}} onClick={post} disabled={posting}>{posting?(pendingFiles.length>0?'Posting & uploading...':'Posting...'):'Post Entry'}</button></div>
...
export default function App(){
  ...
  return(<div style={S.app}>
    ...
    {showJE&&activeEntity&&<JournalEntryModal entityId={activeEntity} user={user} onClose={()=>{setShowJE(false);setJeForm(BLANK_JE());setPendingFiles([]);}} onPosted={()=>setRk(k=>k+1)} jeForm={jeForm} setJeForm={setJeForm} pendingFiles={pendingFiles} setPendingFiles={setPendingFiles}/>}
