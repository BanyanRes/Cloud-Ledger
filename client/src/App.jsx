...
// ═══ Reports ═══
function TrialBalance({entityId,entityName}){
...
function BalanceSheet({entityId,entityName}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const[drillAcct,setDrillAcct]=useState(null);const fyS=asOf.slice(0,4)+'-01-01';
  const drillFrom=useMemo(()=>{const d=new Date(asOf+'T00:00:00');d.setFullYear(d.getFullYear()-1);d.setDate(d.getDate()+1);return d.toISOString().slice(0,10);},[asOf]);
  useEffect(()=>{api.getBalances(entityId,{as_of:asOf,close_pl_before:fyS}).then(setBalances);},[entityId,asOf,fyS]);
...
function IncomeStatement({entityId,entityName}){const[balances,setBalances]=useState([]);const[from,setFrom]=useState(fy_start());const[to,setTo]=useState(today());const[drillAcct,setDrillAcct]=useState(null);
