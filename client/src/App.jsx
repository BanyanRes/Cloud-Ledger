...
function WorkpapersModal({entity, user, onClose}){
  ...
  const breadcrumbs = useMemo(() => {
    if (!curPath) return [{ label: 'Workpapers', path: '' }];
    const parts = curPath.split('/');
    return [{ label: 'Workpapers', path: '' }, ...parts.map((_, i) => ({ label: parts[i], path: parts.slice(0, i + 1).join('/') }))];
  }, [curPath]);

  const doUpload = async e => {
    const fileList = e.target.files; if (!fileList || fileList.length === 0) return;
    e.target.value = ''; setErr(''); setMsg(''); setUploading(true);
    try { const r = await api.uploadEntityFiles(entity.id, fileList, curPath); setMsg(r.uploaded + ' file(s) uploaded'); load(); }
  ...
  return (<div style={S.modal} onClick={onClose}><div style={{...S.modalBox, maxWidth: 900, maxHeight: '90vh', display: 'flex', flexDirection: 'column'}} onClick={e => e.stopPropagation()}>
    ...
    <div style={{marginBottom: 16}}>
      <div style={{fontSize: 18, fontWeight: 700, color: T.textBright, display: 'flex', alignItems: 'center', gap: 10}}>
        <span style={{fontSize: 22}}>📁</span> Workpapers
      </div>
      <div style={{fontSize: 12, color: T.textMuted, marginTop: 2}}>{entity.name}</div>
    </div>
    {/* Breadcrumbs */}
    ...
    {/* Actions */}
    {canEdit && <div style={{display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap'}}>
      <div style={{position: 'relative', display: 'inline-block', overflow: 'hidden'}}>
        <button style={{...S.btnP, pointerEvents: 'none', opacity: uploading ? 0.6 : 1}}>{uploading ? 'Uploading...' : '+ Upload Files'}</button>
        <input type="file" multiple disabled={uploading} style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0, cursor: uploading ? 'not-allowed' : 'pointer'}} onChange={doUpload}/>
      </div>
      {!newFolderMode
    ...
    <div style={{flex: 1, overflowY: 'auto', border: '1px solid ' + T.border, borderRadius: T.radiusSm}}>
      {loading ? <div style={{padding: 40, textAlign: 'center', color: T.textMuted}}>Loading...</div>
       : (childFolders.length === 0 && currentFiles.length === 0)
         ? <div style={{padding: 40, textAlign: 'center', color: T.textDim}}>This folder is empty{canEdit ? '. Upload files or create a subfolder to get started.' : '.'}</div>
         : <table style={S.table}>
            ...
            <tbody>
              {childFolders.map(fp => <tr key={'d-' + fp} style={{background: T.bgElevated}}>
                ...
                <td style={S.td}>{canEdit && <button style={{...S.btnGhost, color: T.red, fontSize: 11}} onClick={() => deleteFolder(fp)}>Delete</button>}</td>
              </tr>)}
...
function EditJEModal({entityId,entry,accounts:initAccounts,onClose,onSaved}){
  ...
  return(<div style={S.modal}><div style={{...S.modalBox,maxWidth:960}} onClick={e=>e.stopPropagation()}>
    ...
    {(entry.created_by||entry.created_at)&&<div style={{fontSize:11,color:T.textMuted,marginBottom:16}}>
      Posted{entry.created_by?' by '+entry.created_by:''}{entry.created_at?' on '+new Date(entry.created_at+(entry.created_at.includes('Z')||entry.created_at.includes('+')?'':'Z')).toLocaleString('en-US',{timeZone:'America/Los_Angeles',year:'numeric',month:'short',day:'numeric',hour:'numeric',minute:'2-digit',hour12:true,timeZoneName:'short'}):''}
    </div>}
    <div style={{background:T.bgElevated,border:'1px solid '+T.border,borderRadius:T.radiusSm,padding:18,marginBottom:16}}>
      <div style={S.row}><div style={{...S.col,maxWidth:170}}><label style={S.label}>Date</label><input style={S.input} type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))}/></div>
      ...
      return(<div key={acct.code} style={S.cardFlush}><div style={{padding:'14px 20px',display:'flex',alignItems:'center',gap:10,borderBottom:'1px solid '+T.border}}>
        ...
        <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>JE</th><th style={S.th}>Memo</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th><th style={S.thR}>Balance</th><th style={{...S.th,width:100}}>Docs</th></tr></thead>
          <tbody>{txns.map((t,i)=>{run+=dr?(t.debit-t.credit):(t.credit-t.debit);const atts=entryAtts[t.jeId];return<tr key={i}><td style={{...S.td,color:T.textMuted}}>{t.date}</td><td style={S.td}><button style={{background:'none',border:0,padding:0,color:T.accent,fontWeight:600,cursor:'pointer',fontSize:'inherit',fontFamily:'inherit'}} onClick={()=>{const e=entries.find(x=>x.id===t.jeId);if(e)setEditEntry(e);}}>JE-{String(t.jeNum).padStart(4,'0')}</button></td><td style={S.td}>{t.memo}</td><td style={S.tdR}>{t.debit>0?fmt(t.debit):''}</td><td style={S.tdR}>{t.credit>0?fmt(t.credit):''}</td><td style={{...S.tdR,fontWeight:600,color:T.textBright}}>{fmt(run)}</td>
            <td style={S.td}>{atts?atts.map(a=><a key={a.id} href={api.downloadAttachment(a.id)} target="_blank" rel="noreferrer" style={S.attachLink}>{a.original_name}</a>):''}</td></tr>;})}</tbody></table>}</div>);})}
    ...
    {selAcct&&filteredTxns.length>0&&<div style={S.cardFlush}>
      ...
      <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>Description</th><th style={S.thR}>Amount</th><th style={{...S.th,width:240}}>GL Account</th><th style={{...S.th,width:160}}>Memo</th><th style={{...S.th,width:70}}>Status</th><th style={{...S.th,width:36}}></th></tr></thead>
        <tbody>{filteredTxns.map(t=><tr key={t.id} style={t.status==='posted'?{opacity:0.45}:{}}>
          ...
          <td style={{...S.td,padding:'4px 6px'}}>{t.status==='posted'
            ...
            : (t.splits && t.splits.length>0
                ...
                : <div style={{display:'flex',gap:4,alignItems:'center'}}>
                    <div style={{flex:1}}><AccountAutocomplete accounts={accounts} value={t.account_code||''} exclude={selAcct} onChange={v=>codeTransaction(t.id,v,t.memo)} placeholder="Search GL account..."/></div>
...
function TrialBalance({entityId,entityName,asOf,setAsOf}){
  ...
  return(<div><div style={S.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    ...
    <table style={S.table}><thead><tr><th style={S.th}>Code</th><th style={S.th}>Account</th><th style={S.th}>Type</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th></tr></thead>
      <tbody>{rows.map(r=><tr key={r.code}><td style={{...S.td,fontWeight:600,color:T.textBright}}>{r.code}</td><td style={S.td}>{r.name}</td><td style={S.td}><span style={S.tag(r.type)}>{r.type}</span></td>
