  ...
  return(<div style={S.app}>
    ...
    <div style={S.body}><div style={S.sidebar(sidebarCol)}>
      ...
      <div style={S.main}>{(()=>{const en=entities.find(e=>e.id===activeEntity);const entityName=en?en.name:'';return<>
        {page==='dashboard'&&<Dashboard entityId={activeEntity} setActiveEntity={setActiveEntity} setPage={setPage} key={rk}/>}
...
// ═══ Dashboard ═══
function Dashboard({entityId,setActiveEntity,setPage}){const[summary,setSummary]=useState([]);useEffect(()=>{api.getSummary().then(setSummary);},[]);const curr=summary.find(e=>e.id===entityId);
  const go=id=>{setActiveEntity(id);setPage('journal');};
  return(<div><div style={S.h1}>Dashboard</div><div style={S.sub}>{summary.length} entities under management &middot; click a row to open</div>
    <div style={S.cardFlush}><div style={{padding:'18px 20px'}}><div style={S.h2}>All Entities</div></div><table style={S.table}><thead><tr><th style={S.th}>Entity</th><th style={S.thR}>Assets</th><th style={S.thR}>Liabilities</th><th style={S.thR}>Net Income</th><th style={S.thR}>JEs</th></tr></thead>
      <tbody>{summary.sort((a,b)=>a.name.localeCompare(b.name)).map(e=><tr key={e.id} onClick={()=>go(e.id)} style={{cursor:'pointer',background:e.id===entityId?T.accentDim:'transparent',transition:'background 0.1s'}} onMouseEnter={ev=>{if(e.id!==entityId)ev.currentTarget.style.background=T.bgHover;}} onMouseLeave={ev=>{if(e.id!==entityId)ev.currentTarget.style.background='transparent';}}><td style={{...S.td,fontWeight:600,color:T.textBright}}>{e.name}</td><td style={S.tdR}>{fmt(e.assets)}</td><td style={S.tdR}>{fmt(e.liabilities)}</td><td style={{...S.tdR,color:e.net_income>=0?T.green:T.red,fontWeight:600}}>{fmt(e.net_income)}</td><td style={S.tdR}>{e.entry_count}</td></tr>)}</tbody></table></div></div>);}
