...
const JE_DRAFT_KEY = 'cl_je_draft';
const JE_FILES_META_KEY = 'cl_je_files_meta';
const SIDEBAR_KEY = 'cl_sidebar';

// Draft persistence
function saveDraft(entityId, form) { try { localStorage.setItem(JE_DRAFT_KEY, JSON.stringify({ entityId, form, savedAt: Date.now() })); } catch {} }
function loadDraft(entityId) { try { const raw = localStorage.getItem(JE_DRAFT_KEY); if (!raw) return null; const d = JSON.parse(raw); if (d.entityId !== entityId) return null; if (Date.now() - d.savedAt > 7*24*60*60*1000) { localStorage.removeItem(JE_DRAFT_KEY); return null; } return d.form; } catch { return null; } }
function clearDraft() { try { localStorage.removeItem(JE_DRAFT_KEY); localStorage.removeItem(JE_FILES_META_KEY); } catch {} }

// Simple keyword-based GL suggestion
const CATEGORY_HINTS = [
  { keywords: ['payroll','salary','wage','bonus','compensation'], account_subtypes: ['Operating Expense'], prefer_name: /salari|payroll/i },
  { keywords: ['rent','lease','office space'], account_subtypes: ['Operating Expense'], prefer_name: /rent/i },
  { keywords: ['electric','gas','water','utility','utilities','power'], account_subtypes: ['Operating Expense'], prefer_name: /utilit/i },
  { keywords: ['insurance','premium','policy'], account_subtypes: ['Operating Expense'], prefer_name: /insurance/i },
  { keywords: ['supplies','office depot','staples','amazon'], account_subtypes: ['Operating Expense'], prefer_name: /supplies|office/i },
  { keywords: ['advertising','marketing','google ads','facebook','meta','ad spend'], account_subtypes: ['Operating Expense'], prefer_name: /market/i },
  { keywords: ['interest','loan payment','finance charge'], account_subtypes: ['Other Expense'], prefer_name: /interest/i },
  { keywords: ['deposit','payment received','wire in','ach credit','revenue','sales','income'], account_subtypes: ['Operating Revenue','Other Revenue'], prefer_name: /revenue|income/i },
  { keywords: ['depreciation','amortization'], account_subtypes: ['Operating Expense'], prefer_name: /deprec/i },
];

function suggestAccount(description, accounts, bankAcctCode) {
  if (!description) return null;
  const desc = description.toLowerCase();
  for (const hint of CATEGORY_HINTS) {
    if (hint.keywords.some(k => desc.includes(k))) {
      const candidates = accounts.filter(a => a.code !== bankAcctCode && hint.account_subtypes.includes(a.subtype));
      const preferred = candidates.find(a => hint.prefer_name.test(a.name));
...
// ─── Design Tokens ───
const T = {
  bg: '#0a0c10', bgCard: '#111318', bgCardHover: '#161921', bgElevated: '#1a1d26',
  border: '#1e2230', borderLight: '#282d3a', borderFocus: '#3b82f640',
  text: '#d1d5db', textBright: '#f3f4f6', textMuted: '#6b7280', textDim: '#4b5563',
  accent: '#3b82f6', accentLight: '#60a5fa', accentDim: '#3b82f618',
  green: '#10b981', greenDim: '#10b98118', greenBorder: '#10b98130',
  red: '#ef4444', redDim: '#ef444418', orange: '#f59e0b', orangeDim: '#f59e0b18',
  purple: '#8b5cf6', purpleDim: '#8b5cf618', teal: '#14b8a6', tealDim: '#14b8a618',
  shadow: '0 1px 3px rgba(0,0,0,0.3), 0 1px 2px rgba(0,0,0,0.2)',
  shadowLg: '0 4px 12px rgba(0,0,0,0.4), 0 2px 4px rgba(0,0,0,0.3)',
...
const S = {
  ...
  topBar: { background: T.bgCard, borderBottom: '1px solid '+T.border, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, zIndex: 10 },
  body: { display: 'flex', flex: 1, overflow: 'hidden' },
  sidebar: (collapsed) => ({ width: collapsed ? 56 : 220, background: T.bgCard, borderRight: '1px solid '+T.border, padding: collapsed ? '12px 4px' : '12px 0', flexShrink: 0, overflowY: 'auto', overflowX: 'hidden', transition: 'width 0.2s ease', display: 'flex', flexDirection: 'column' }),
  navItem: (active, collapsed) => ({ padding: collapsed ? '10px 0' : '9px 20px', cursor: 'pointer', fontSize: 12.5, fontWeight: active ? 600 : 400, color: active ? T.textBright : T.textMuted, background: active ? T.accentDim : 'transparent', borderRadius: collapsed ? T.radiusXs : 0, margin: collapsed ? '2px 6px' : '1px 8px', borderLeft: collapsed ? 'none' : (active ? '2px solid '+T.accent : '2px solid transparent'), transition: 'all 0.15s', textAlign: collapsed ? 'center' : 'left', whiteSpace: 'nowrap', overflow: 'hidden' }),
  navSection: (collapsed) => ({ padding: collapsed ? '12px 0 4px' : '16px 20px 6px', fontSize: 9, fontWeight: 700, color: T.textDim, textTransform: 'uppercase', letterSpacing: '0.12em', textAlign: collapsed ? 'center' : 'left' }),
  ...
  th: { textAlign: 'left', padding: '10px 14px', borderBottom: '1px solid '+T.border, color: T.textDim, fontWeight: 600, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' },
  thR: { textAlign: 'right', padding: '10px 14px', borderBottom: '1px solid '+T.border, color: T.textDim, fontWeight: 600, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' },
  thC: { textAlign: 'center', padding: '10px 14px', borderBottom: '1px solid '+T.border, color: T.textDim, fontWeight: 600, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em' },
  td: { padding: '10px 14px', borderBottom: '1px solid '+T.border+'80' },
  tdR: { padding: '10px 14px', borderBottom: '1px solid '+T.border+'80', textAlign: 'right', fontVariantNumeric: 'tabular-nums' },
  tdC: { padding: '10px 14px', borderBottom: '1px solid '+T.border+'80', textAlign: 'center' },
  tdBold: { padding: '10px 14px', borderBottom: '2px solid '+T.borderLight, color: T.textBright, fontWeight: 700, fontVariantNumeric: 'tabular-nums' },
  // Form
  input: { background: T.bg, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '9px 12px', color: T.text, fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box', transition: 'border-color 0.15s' },
  inputSm: { background: T.bg, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '6px 10px', color: T.text, fontSize: 12, outline: 'none', boxSizing: 'border-box' },
  select: { background: T.bg, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '9px 12px', color: T.text, fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box' },
  selectSm: { background: T.bg, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '6px 10px', color: T.text, fontSize: 12, outline: 'none', boxSizing: 'border-box' },
  // Buttons
  btnP: { background: T.accent, color: '#fff', border: 'none', borderRadius: T.radiusXs, padding: '9px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.15s' },
  btnS: { background: T.bgElevated, color: T.text, border: '1px solid '+T.border, borderRadius: T.radiusXs, padding: '9px 20px', fontSize: 13, fontWeight: 500, cursor: 'pointer' },
  ...
  label: { fontSize: 11, color: T.textMuted, marginBottom: 4, display: 'block', fontWeight: 500 },
  err: { color: T.red, fontSize: 12, marginTop: 6 }, success: { color: T.green, fontSize: 12, marginTop: 6 },
  tag: t => { const c = { Asset: T.accent, Liability: T.orange, Equity: T.green, Revenue: T.purple, Expense: T.red }; return { display: 'inline-block', padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 600, color: c[t]||T.textDim, background: (c[t]||T.textDim)+'18' }; },
  ...
  // Modal
  modal: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  modalBox: { background: T.bgCard, border: '1px solid '+T.border, borderRadius: '14px', width: '94%', maxWidth: 960, maxHeight: '92vh', overflowY: 'auto', padding: 28, position: 'relative', boxShadow: T.shadowLg },
  ...
  sectionHeader: { background: T.bgElevated, padding: '8px 14px', fontWeight: 600, color: T.textBright, fontSize: 12 },
  indentTd: { padding: '8px 14px 8px 28px', borderBottom: '1px solid '+T.border+'40', fontSize: 13 },
  subtotalRow: { borderTop: '1px solid '+T.borderLight },
  grandTotalRow: { borderTop: '2px solid '+T.borderLight, background: T.bgElevated },
  // Stat cards
  statCard: { textAlign: 'center', padding: 20, background: T.bgCard, border: '1px solid '+T.border, borderRadius: T.radius, boxShadow: T.shadow },
...
const NAV_ICONS = { dashboard:'\u25a3', journal:'\u270e', coa:'\u2630', ledger:'\u2261', banktxn:'\u21c5', bankrec:'\u2611', trial:'\u2696', bs:'\u25a6', is:'\u25a4', entities:'\u2302', users:'\u263a' };
...
function AccountAutocomplete({ accounts, value, onChange, placeholder, exclude }) {
  ...
  const filtered = useMemo(() => {
    ...
    return accounts.filter(a => (!exclude || a.code !== exclude) && (a.code.toLowerCase().includes(q) || a.name.toLowerCase().includes(q))).slice(0, 12);
  }, [accounts, query, exclude]);
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <input style={S.inputSm} placeholder={placeholder || 'Search account...'} value={open ? query : (selected ? acctLabel(selected.code, selected.name) : '')}
        onFocus={() => { setOpen(true); setQuery(''); }}
        onChange={e => { setQuery(e.target.value); setOpen(true); }}
        onKeyDown={e => { if (e.key === 'Escape') setOpen(false); if (e.key === 'Enter' && filtered.length > 0) { onChange(filtered[0].code); setOpen(false); } }}
      />
      {!value && !open && <span style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', color: T.textDim, fontSize: 10 }}>Type to search</span>}
      {open && filtered.length > 0 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: T.bgCard, border: '1px solid '+T.border, borderRadius: T.radiusSm, maxHeight: 220, overflowY: 'auto', zIndex: 50, boxShadow: T.shadowLg, marginTop: 4 }}>
          {filtered.map(a => (
            <div key={a.code} style={{ padding: '8px 12px', cursor: 'pointer', fontSize: 12, display: 'flex', justifyContent: 'space-between', background: a.code === value ? T.accentDim : 'transparent' }}
              onClick={() => { onChange(a.code); setOpen(false); }}
              onMouseEnter={e => e.currentTarget.style.background = T.bgElevated}
              onMouseLeave={e => e.currentTarget.style.background = a.code === value ? T.accentDim : 'transparent'}>
              <span><span style={{ fontWeight: 600, color: T.textBright }}>{a.code}</span> <span style={{ color: T.textMuted }}>{a.name}</span></span>
              <span style={S.tag(a.type)}>{a.type}</span>
            </div>
          ))}
        </div>
      )}
...
function AuthScreen({onLogin}){const[mode,setMode]=useState('login');const[email,setEmail]=useState('');const[pw,setPw]=useState('');const[name,setName]=useState('');const[confirmPw,setConfirmPw]=useState('');const[role,setRole]=useState('Accountant');
  ...
  return(<div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:T.bg}}>
    <div style={{background:T.bgCard,border:'1px solid '+T.border,borderRadius:16,width:420,padding:44,textAlign:'center',boxShadow:T.shadowLg}}>
      <div style={{...S.logoIcon,width:48,height:48,fontSize:20,margin:'0 auto 16px'}}>GL</div>
      <div style={{fontSize:24,fontWeight:800,color:T.textBright,marginBottom:4,letterSpacing:'-0.03em'}}>CloudLedger</div>
      ...
      {mode==='forgot'?(<>
        ...
        {tempPw&&<div style={{background:T.greenDim,border:'1px solid '+T.greenBorder,borderRadius:T.radiusSm,padding:20,margin:'12px 0'}}>
          <div style={{fontSize:12,color:T.green,marginBottom:8}}>Your temporary password:</div>
          <div style={{fontSize:20,fontWeight:700,color:T.textBright,fontFamily:'monospace',letterSpacing:2}}>{tempPw}</div>
          <div style={{fontSize:11,color:T.textMuted,marginTop:10}}>Sign in, then change it in Settings.</div></div>}
        <button style={{...S.btnP,width:'100%',padding:11,marginTop:8}} onClick={doForgot} disabled={loading}>{loading?'Resetting...':'Reset Password'}</button>
        ...
        <div style={{display:'flex',marginBottom:24,borderRadius:T.radiusSm,overflow:'hidden',border:'1px solid '+T.border}}>
          <div onClick={()=>{setMode('login');setErr('');}} style={{flex:1,padding:'10px 0',cursor:'pointer',fontSize:13,fontWeight:600,textAlign:'center',background:mode==='login'?T.accentDim:'transparent',color:mode==='login'?T.accent:T.textMuted,transition:'all 0.15s'}}>Sign In</div>
          <div onClick={()=>{setMode('signup');setErr('');}} style={{flex:1,padding:'10px 0',cursor:'pointer',fontSize:13,fontWeight:600,textAlign:'center',background:mode==='signup'?T.greenDim:'transparent',color:mode==='signup'?T.green:T.textMuted,transition:'all 0.15s'}}>Create Account</div></div>
        {mode==='login'?(<>
          ...
          {err&&<div style={S.err}>{err}</div>}
          <button style={{...S.btnP,width:'100%',padding:11,marginTop:8}} onClick={doLogin} disabled={loading}>{loading?'Signing in...':'Sign In'}</button>
          ...
          <button style={{...S.btnP,width:'100%',padding:11,marginTop:8,background:T.green}} onClick={doSignup} disabled={loading}>{loading?'Creating...':'Create Account'}</button>
...
function ChangePasswordModal({onClose}){const[cur,setCur]=useState('');const[nw,setNw]=useState('');const[cf,setCf]=useState('');const[err,setErr]=useState('');const[ok,setOk]=useState(false);
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:400,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
    <button style={S.modalClose} onClick={onClose}>&times;</button><div style={{fontSize:18,fontWeight:700,color:T.textBright,marginBottom:20}}>Change Password</div>
    <div style={{marginBottom:12}}><input style={S.input} type="password" placeholder="Current password" value={cur} onChange={e=>{setCur(e.target.value);setErr('');}}/></div>
    <div style={{marginBottom:12}}><input style={S.input} type="password" placeholder="New password" value={nw} onChange={e=>{setNw(e.target.value);setErr('');}}/></div>
    <div style={{marginBottom:12}}><input style={S.input} type="password" placeholder="Confirm new" value={cf} onChange={e=>{setCf(e.target.value);setErr('');}}/></div>
    {err&&<div style={S.err}>{err}</div>}{ok&&<div style={S.success}>Password updated!</div>}
    <button style={{...S.btnP,width:'100%',padding:11,marginTop:8}} onClick={async()=>{if(nw.length<3){setErr('Min 3 chars');return;}if(nw!==cf){setErr("Don't match");return;}try{await api.changePassword(cur,nw);setOk(true);setTimeout(onClose,1500);}catch(e){setErr(e.message);}}}>Update Password</button>
  </div></div>);}
...
function QuickAddAccountModal({entityId,onClose,onCreated}){const[form,setForm]=useState({code:'',name:'',type:'Asset',subtype:'',bank_acct:false});const[err,setErr]=useState('');
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:640}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={S.row}><div style={S.col}><label style={S.label}>Account Code</label><input style={S.input} placeholder="e.g. 61500" value={form.code} onChange={e=>setForm(f=>({...f,code:e.target.value}))}/></div>
      <div style={{...S.col,flex:2}}><label style={S.label}>Account Name</label><input style={S.input} value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></div>
    ...
    <div style={{marginBottom:14}}><label style={{display:'flex',alignItems:'center',gap:8,fontSize:13,cursor:'pointer',color:T.text}}><input type="checkbox" style={S.checkbox} checked={form.bank_acct} onChange={e=>setForm(f=>({...f,bank_acct:e.target.checked}))}/>Bank / cash account</label></div>
    {err&&<div style={S.err}>{err}</div>}
    <div style={{display:'flex',gap:10,marginTop:12}}><button style={S.btnP} onClick={async()=>{if(!form.code||!form.name){setErr('Code and name required');return;}try{const a=await api.createAccount(entityId,form);onCreated(a);onClose();}catch(e){setErr(e.message);}}}>Add Account</button><button style={S.btnS} onClick={onClose}>Cancel</button></div>
  </div></div>);}

// ─── Journal Entry Modal (with localStorage draft persistence) ───
function JournalEntryModal({entityId,user,onClose,onPosted}){
  ...
  const[pendingFiles,setPendingFiles]=useState([]);const[draftStatus,setDraftStatus]=useState('');
  // Load draft from localStorage on mount
  const[form,setForm]=useState(()=>loadDraft(entityId)||BLANK_JE());
  useEffect(()=>{api.getAccounts(entityId).then(setAccounts);},[entityId]);
  // Auto-save draft on every form change
  useEffect(()=>{const t=setTimeout(()=>{const hasContent=form.memo||form.lines.some(l=>l.account_code||l.debit||l.credit);if(hasContent){saveDraft(entityId,form);setDraftStatus('Draft saved');setTimeout(()=>setDraftStatus(''),2000);}},500);return()=>clearTimeout(t);},[form,entityId]);
  ...
  const discardDraft=()=>{setForm(BLANK_JE());setPendingFiles([]);clearDraft();setDraftStatus('Draft discarded');setTimeout(()=>setDraftStatus(''),2000);};
  const post=async()=>{if(!form.date||!form.memo.trim()){setErr('Date and memo required');return;}if(form.lines.some(l=>!l.account_code)){setErr('All lines need an account');return;}if(!bal){setErr('Entry must balance');return;}
    setPosting(true);setErr('');try{const r=await api.createEntry(entityId,{date:form.date,memo:form.memo.trim(),lines:form.lines.map(l=>({account_code:l.account_code,debit:parseFloat(l.debit)||0,credit:parseFloat(l.credit)||0}))});
      ...
      setForm(BLANK_JE());setPendingFiles([]);clearDraft();setPosted(msg+'!');setTimeout(()=>setPosted(''),5000);if(onPosted)onPosted();}
    catch(e){setErr(e.message);}finally{setPosting(false);}};
  const handleClose=()=>{clearDraft();onClose();};

  return(<div style={S.modal} onClick={handleClose}><div style={{...S.modalBox,maxWidth:980}} onClick={e=>e.stopPropagation()}>
    <button style={S.modalClose} onClick={handleClose}>&times;</button>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
      <div style={{fontSize:18,fontWeight:700,color:T.textBright}}>New Journal Entry</div>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        {draftStatus&&<span style={{fontSize:11,color:draftStatus.includes('saved')?T.green:T.orange}}>{draftStatus}</span>}
        <button style={{...S.btnGhost,color:T.red}} onClick={discardDraft}>Discard draft</button>
      </div>
    </div>
    {/* Header fields */}
    <div style={{...S.card,padding:18,marginBottom:16,background:T.bg}}>
    ...
    {/* Lines */}
    <div style={{...S.cardFlush,marginBottom:16}}>
      <table style={S.table}><thead><tr><th style={S.th}>Account</th><th style={{...S.thR,width:140}}>Debit</th><th style={{...S.thR,width:140}}>Credit</th><th style={{...S.th,width:36}}></th></tr></thead>
        <tbody>{form.lines.map((l,i)=><tr key={i}><td style={{padding:'6px 8px',borderBottom:'1px solid '+T.border+'80'}}>
          ...
          <td style={{padding:'6px 8px',borderBottom:'1px solid '+T.border+'80'}}><input style={{...S.input,textAlign:'right',fontVariantNumeric:'tabular-nums'}} placeholder="0.00" value={l.debit} onChange={e=>updateLine(i,'debit',e.target.value)}/></td>
          <td style={{padding:'6px 8px',borderBottom:'1px solid '+T.border+'80'}}><input style={{...S.input,textAlign:'right',fontVariantNumeric:'tabular-nums'}} placeholder="0.00" value={l.credit} onChange={e=>updateLine(i,'credit',e.target.value)}/></td>
          <td style={{padding:'6px',borderBottom:'1px solid '+T.border+'80',textAlign:'center'}}>{form.lines.length>2&&<button style={S.btnGhost} onClick={()=>removeLine(i)}>&times;</button>}</td></tr>)}
        <tr style={{background:T.bgElevated}}><td style={{...S.tdBold,textAlign:'right',fontSize:12}}>TOTAL</td>
          <td style={{...S.tdBold,textAlign:'right',fontSize:15}}>${fmt(tDr)}</td>
          <td style={{...S.tdBold,textAlign:'right',fontSize:15}}>${fmt(tCr)}</td><td style={S.tdBold}></td></tr></tbody></table>
    </div>
    {/* Attachments */}
    <div style={{border:'1px solid '+(pendingFiles.length>0?T.teal+'40':T.border),borderRadius:T.radiusSm,padding:16,marginBottom:16,background:pendingFiles.length>0?T.tealDim:'transparent',transition:'all 0.2s'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:pendingFiles.length>0?12:0}}>
        ...
        <div><input type="file" multiple accept=".pdf,.xlsx,.xls,.csv,.png,.jpg,.jpeg,.eml,.msg,.doc,.docx" style={{display:'none'}} id="je-file-input" onChange={e=>{if(e.target.files.length>0)setPendingFiles(p=>[...p,...Array.from(e.target.files)]);e.target.value='';}}/><label htmlFor="je-file-input" style={{...S.btnS,padding:'7px 16px',cursor:'pointer',display:'inline-block'}}>Attach files</label></div>
      </div>
      {pendingFiles.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:6}}>{pendingFiles.map((f,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:6,background:T.bgCard,padding:'6px 12px',borderRadius:6,fontSize:12,border:'1px solid '+T.border}}>
        <span style={{color:T.textBright,fontWeight:500}}>{f.name}</span><span style={{color:T.textDim}}>({fmtSize(f.size)})</span>
        <button style={{...S.btnGhost,color:T.red,padding:0,fontSize:14}} onClick={()=>setPendingFiles(p=>p.filter((_,j)=>j!==i))}>&times;</button></div>)}</div>}
    </div>
    {/* Actions */}
    <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
      ...
      <button style={{...S.btnP,padding:'10px 28px',fontSize:14,opacity:posting?.6:1}} onClick={post} disabled={posting}>{posting?'Posting...':'Post Entry'}</button>
    </div>
...
function EntityPicker({entities,activeId,onSelect,onManage}){const[open,setOpen]=useState(false);const[search,setSearch]=useState('');const active=entities.find(e=>e.id===activeId);
  ...
  return(<div style={{position:'relative'}}><div style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',padding:'6px 14px',borderRadius:T.radiusSm,background:T.bgElevated,border:'1px solid '+T.border}} onClick={()=>setOpen(!open)}>
    <span style={{fontWeight:700,color:T.textBright,fontSize:13}}>{active?.code||'-'}</span><span style={{color:T.textMuted,fontSize:12,maxWidth:180,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{active?.name||'Select'}</span>
    <span style={{color:T.textDim,fontSize:9,marginLeft:4}}>{'\u25bc'}</span></div>
    {open&&<><div style={{position:'fixed',inset:0,zIndex:50}} onClick={()=>{setOpen(false);setSearch('');}}/>
      <div style={{position:'absolute',top:'100%',left:0,background:T.bgCard,border:'1px solid '+T.border,borderRadius:T.radius,maxHeight:380,overflowY:'auto',zIndex:100,boxShadow:T.shadowLg,width:340,marginTop:6}}>
        <div style={{position:'sticky',top:0,padding:12,background:T.bgCard,borderBottom:'1px solid '+T.border}}><input style={S.input} placeholder={'Search '+entities.length+' entities...'} value={search} onChange={e=>setSearch(e.target.value)} autoFocus/></div>
        {filtered.map(e=><div key={e.id} style={{padding:'10px 16px',cursor:'pointer',background:e.id===activeId?T.accentDim:'transparent',borderLeft:e.id===activeId?'3px solid '+T.accent:'3px solid transparent',transition:'all 0.1s'}} onClick={()=>{onSelect(e.id);setOpen(false);setSearch('');}}>
...
export default function App(){
  ...
  const[sidebarCollapsed,setSidebarCollapsed]=useState(()=>{try{return localStorage.getItem(SIDEBAR_KEY)==='true';}catch{return false;}});

  useEffect(()=>{try{localStorage.setItem(SIDEBAR_KEY,String(sidebarCollapsed));}catch{}},[sidebarCollapsed]);
  ...
  const navItems=[
    {id:'dashboard',label:'Dashboard',icon:NAV_ICONS.dashboard,section:'reports'},
    {id:'d1',divider:1,label:'TRANSACTIONS'},
    {id:'journal',label:'Journal Entries',icon:NAV_ICONS.journal,section:'entries'},
    {id:'d2',divider:1,label:'ACCOUNTS'},
    {id:'coa',label:'Chart of Accounts',icon:NAV_ICONS.coa,section:'coa'},
    {id:'ledger',label:'General Ledger',icon:NAV_ICONS.ledger,section:'reports'},
    {id:'d2b',divider:1,label:'BANKING'},
    {id:'banktxn',label:'Bank Transactions',icon:NAV_ICONS.banktxn,section:'bankrec'},
    {id:'bankrec',label:'Bank Reconciliation',icon:NAV_ICONS.bankrec,section:'bankrec'},
    {id:'d3',divider:1,label:'REPORTS'},
    {id:'trial',label:'Trial Balance',icon:NAV_ICONS.trial,section:'reports'},
    {id:'bs',label:'Balance Sheet',icon:NAV_ICONS.bs,section:'reports'},
    {id:'is',label:'Income Statement',icon:NAV_ICONS.is,section:'reports'},
    {id:'d4',divider:1,label:'ADMIN'},
    {id:'entities',label:'Entities ('+entities.length+')',icon:NAV_ICONS.entities,section:'all'},
    {id:'users',label:'Users',icon:NAV_ICONS.users,section:'all'},
  ];
  ...
  return(<div style={S.app}>
    ...
    <div style={S.topBar}>
      <div style={{display:'flex',alignItems:'center',gap:16}}>
        <button style={{...S.btnGhost,fontSize:18,padding:'4px 6px'}} onClick={()=>setSidebarCollapsed(c=>!c)} title="Toggle sidebar">{sidebarCollapsed?'\u2630':'\u2190'}</button>
        <div style={{display:'flex',alignItems:'center',gap:10}}><div style={S.logoIcon}>GL</div>{!sidebarCollapsed&&<div style={{fontSize:17,fontWeight:800,color:T.textBright,letterSpacing:'-0.03em'}}>CloudLedger</div>}</div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        {canAccess('entries')&&activeEntity&&<button style={S.btnP} onClick={()=>setShowJE(true)}>+ Journal Entry</button>}
        <span style={{fontSize:13,color:T.text,fontWeight:500}}>{user.name}</span><span style={S.badge}>{user.role}</span>
        <button style={{...S.btnS,padding:'7px 14px'}} onClick={()=>setShowChangePw(true)}>Settings</button>
        <button style={{...S.btnS,padding:'7px 14px'}} onClick={()=>{api.clearToken();setUser(null);}}>Sign Out</button>
      </div>
    </div>
    <div style={S.body}>
      {/* Sidebar */}
      <div style={S.sidebar(sidebarCollapsed)}>
        {navItems.map(n=>n.divider?(!sidebarCollapsed?<div key={n.id} style={S.navSection(sidebarCollapsed)}>{n.label}</div>:<div key={n.id} style={{height:8}}/>)
          :(n.section==='all'?user.role==='Admin':canAccess(n.section))?<div key={n.id} style={S.navItem(page===n.id,sidebarCollapsed)} onClick={()=>setPage(n.id)} title={n.label}>
            {sidebarCollapsed?<span style={{fontSize:15}}>{n.icon}</span>:<span>{n.icon} {n.label}</span>}
          </div>:null)}
      </div>
      {/* Main */}
      <div style={S.main}>
    ...
    {showJE&&activeEntity&&<JournalEntryModal entityId={activeEntity} user={user} onClose={()=>setShowJE(false)} onPosted={()=>setRk(k=>k+1)}/>}
...
function Dashboard({entityId}){const[summary,setSummary]=useState([]);useEffect(()=>{api.getSummary().then(setSummary);},[]);const curr=summary.find(e=>e.id===entityId);
  return(<div><div style={S.h1}>Dashboard</div><div style={S.sub}>{summary.length} entities under management</div>
    {curr&&<div style={S.summaryBar}>
      {[{l:'Total Assets',v:curr.assets,c:T.accent},{l:'Total Liabilities',v:curr.liabilities,c:T.orange},{l:'Revenue',v:curr.revenue,c:T.purple},{l:'Expenses',v:curr.expenses,c:T.red},{l:'Net Income',v:curr.net_income,c:curr.net_income>=0?T.green:T.red},{l:'Entries',v:curr.entry_count,c:T.textMuted,raw:1}].map(s=>
...
function JournalList({entityId,onNewEntry}){const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);const[from,setFrom]=useState('');const[to,setTo]=useState('');
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>Journal Entries</div><div style={{fontSize:13,color:T.textMuted}}>{entries.length} entries</div></div><button style={S.btnP} onClick={onNewEntry}>+ New Entry</button></div>
      ...
      entries.map(e=><div key={e.id} style={{...S.card,padding:18}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            <span style={{fontWeight:700,color:T.accent,fontSize:14}}>JE-{String(e.entry_num).padStart(4,'0')}</span>
            <span style={{color:T.textMuted,fontSize:13}}>{e.date}</span>
            <span style={{color:T.text,fontSize:13,fontWeight:500}}>{e.memo}</span>
            {e.attachments?.length>0&&<span style={{fontSize:11,color:T.teal,fontWeight:500}}>({e.attachments.length} attachment{e.attachments.length>1?'s':''})</span>}</div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <span style={{fontSize:12,color:T.textDim}}>{e.created_by}</span>
            <button style={S.btnD} onClick={()=>del(e.id)}>Delete</button></div></div>
...
function ChartOfAccounts({entityId,canEdit}){const[accounts,setAccounts]=useState([]);const[showAdd,setShowAdd]=useState(false);
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>Chart of Accounts</div><div style={{fontSize:13,color:T.textMuted}}>{accounts.length} accounts</div></div>
    ...
    <div style={S.cardFlush}><table style={S.table}><thead><tr><th style={S.th}>Code</th><th style={S.th}>Name</th><th style={S.th}>Type</th><th style={S.th}>Subtype</th><th style={S.thC}>Bank</th>{canEdit&&<th style={{...S.th,width:40}}></th>}</tr></thead>
      <tbody>{accounts.map(a=><tr key={a.code}><td style={{...S.td,fontWeight:600,color:T.textBright}}>{a.code}</td><td style={S.td}>{a.name}</td><td style={S.td}><span style={S.tag(a.type)}>{a.type}</span></td><td style={{...S.td,color:T.textDim}}>{a.subtype}</td>
        <td style={S.tdC}>{a.bank_acct?<span style={{color:T.green}}>Yes</span>:''}</td>{canEdit&&<td style={S.td}><button style={S.btnGhost} onClick={async()=>{try{await api.deleteAccount(entityId,a.code);load();}catch(e){alert(e.message);}}}>x</button></td>}</tr>)}</tbody></table></div></div>);}
...
function GeneralLedger({entityId}){const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);const[filter,setFilter]=useState('');const[from,setFrom]=useState(fy_start());const[to,setTo]=useState(today());
  ...
  const entryAttachments={};entries.forEach(e=>{if(e.attachments?.length>0)entryAttachments[e.id]=e.attachments;});
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><div style={S.h1}>General Ledger</div><button style={S.btnExport} onClick={doExport}>Export Excel</button></div><div style={S.sub}/>
    <div style={S.filterBar}><div><label style={S.label}>From</label><input style={S.inputSm} type="date" value={from} onChange={e=>setFrom(e.target.value)}/></div>
      <div><label style={S.label}>To</label><input style={S.inputSm} type="date" value={to} onChange={e=>setTo(e.target.value)}/></div>
    ...
    {filtered.map(acct=>{const txns=[];entries.forEach(e=>{e.lines.forEach(l=>{if(l.account_code===acct.code)txns.push({...l,date:e.date,memo:e.memo,jeNum:e.entry_num,jeId:e.id});});});
      ...
      return(<div key={acct.code} style={S.cardFlush}><div style={{padding:'14px 20px',display:'flex',alignItems:'center',gap:10,borderBottom:'1px solid '+T.border}}>
        <span style={{fontWeight:700,color:T.textBright,fontSize:14}}>{acct.code}</span><span style={{color:T.text}}>{acct.name}</span><span style={S.tag(acct.type)}>{acct.type}</span></div>
        ...
        <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>JE</th><th style={S.th}>Memo</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th><th style={S.thR}>Balance</th><th style={{...S.th,width:100}}>Docs</th></tr></thead>
          <tbody>{txns.map((t,i)=>{run+=dr?(t.debit-t.credit):(t.credit-t.debit);const atts=entryAttachments[t.jeId];return<tr key={i}><td style={{...S.td,color:T.textMuted}}>{t.date}</td><td style={S.td}><span style={{color:T.accent,fontWeight:600}}>JE-{String(t.jeNum).padStart(4,'0')}</span></td><td style={S.td}>{t.memo}</td><td style={S.tdR}>{t.debit>0?fmt(t.debit):''}</td><td style={S.tdR}>{t.credit>0?fmt(t.credit):''}</td><td style={{...S.tdR,fontWeight:600,color:T.textBright}}>{fmt(run)}</td>
...
// ═══ Bank Transactions (with autocomplete, suggestions, summary, cancel upload) ═══
function BankTransactions({entityId}){const[accounts,setAccounts]=useState([]);const[bankAccts,setBankAccts]=useState([]);const[txns,setTxns]=useState([]);
  ...
  const[uploading,setUploading]=useState(false);const uploadAbort=useRef(null);
  ...
  const handleUpload=async e=>{const file=e.target.files[0];if(!file||!selAcct)return;e.target.value='';setErr('');setMsg('');setUploading(true);
    try{const r=await api.uploadBankTransactions(entityId,selAcct,file);
      // Auto-suggest accounts for imported transactions
      const imported=await api.getBankTransactions(entityId,selAcct,'pending');
      let autoCodedCount=0;
      for(const t of imported){if(!t.account_code){const suggestion=suggestAccount(t.description,accounts,selAcct);
        if(suggestion){await api.codeBankTransaction(entityId,t.id,suggestion.code,t.memo||t.description);autoCodedCount++;}}}
      setMsg(r.count+' transactions imported'+(autoCodedCount>0?', '+autoCodedCount+' auto-categorized':''));load();
    }catch(ex){if(ex.name!=='AbortError')setErr(ex.message);}finally{setUploading(false);}};
  const cancelUpload=async()=>{setUploading(false);setMsg('Upload cancelled');};
  const codeTransaction=async(id,account_code,memo)=>{await api.codeBankTransaction(entityId,id,account_code,memo);load();};
  const postCoded=async()=>{const ids=txns.filter(t=>t.status==='coded').map(t=>t.id);if(!ids.length){setErr('No coded transactions');return;}try{const r=await api.postBankTransactions(entityId,ids);setMsg(r.posted+' journal entries created');load();}catch(ex){setErr(ex.message);}};
  const handleAcctCreated=a=>{setAccounts(p=>[...p,a].sort((x,y)=>x.code.localeCompare(y.code)));if(a.bank_acct)setBankAccts(p=>[...p,a].sort((x,y)=>x.code.localeCompare(y.code)));};
  ...
  const uncategorized=txns.filter(t=>t.status==='pending').length;
  ...
  return(<div><div style={S.h1}>Bank Transactions</div><div style={S.sub}>Upload, categorize, and post bank activity to the general ledger</div>
    ...
    <div style={S.card}><div style={S.row}>
      <div style={{...S.col,flex:2}}><label style={S.label}>Bank Account</label><select style={S.select} value={selAcct} onChange={e=>{setSelAcct(e.target.value);setTxns([]);}}><option value="">Select bank account...</option>{bankAccts.map(a=><option key={a.code} value={a.code}>{acctLabel(a.code,a.name)}</option>)}</select></div>
      <div style={S.col}><label style={S.label}>Status</label><select style={S.select} value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="">All</option><option value="pending">Pending</option><option value="coded">Coded</option><option value="posted">Posted</option></select></div>
      {selAcct&&<div style={{display:'flex',gap:8,alignItems:'flex-end'}}>
        {uploading?<button style={{...S.btnD,padding:'9px 20px'}} onClick={cancelUpload}>Cancel Upload</button>
          :<><input type="file" accept=".csv,.xlsx,.xls" id="bank-upload" style={{display:'none'}} onChange={handleUpload}/><label htmlFor="bank-upload" style={{...S.btnP,cursor:'pointer',display:'inline-block'}}>Upload CSV / Excel</label></>}
      </div>}
    </div>{err&&<div style={S.err}>{err}</div>}{msg&&<div style={S.success}>{msg}</div>}</div>

    {/* Summary bar */}
    {selAcct&&txns.length>0&&<div style={S.summaryBar}>
      <div style={S.summaryItem}><div style={{...S.statVal,fontSize:20,color:T.textBright}}>{txns.length}</div><div style={S.statLabel}>Transactions</div></div>
      <div style={S.summaryItem}><div style={{...S.statVal,fontSize:20,color:T.orange}}>{uncategorized}</div><div style={S.statLabel}>Uncategorized</div></div>
      ...
      <div style={S.summaryItem}><div style={{...S.statVal,fontSize:20,color:T.textBright}}>${fmt(totalIn-totalOut)}</div><div style={S.statLabel}>Net</div></div>
    </div>}

    {/* Transaction list */}
    {selAcct&&txns.length>0&&<div style={S.cardFlush}>
      <div style={{padding:'16px 20px',display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+T.border}}>
        ...
        <div style={{display:'flex',gap:10}}>
          <button style={{...S.btnS,color:T.teal,borderColor:T.teal+'40'}} onClick={()=>setShowAddAcct(true)}>+ New Account</button>
          {txns.some(t=>t.status==='coded')&&<button style={S.btnP} onClick={postCoded}>Post {txns.filter(t=>t.status==='coded').length} to GL</button>}</div></div>
      <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>Description</th><th style={S.thR}>Amount</th><th style={{...S.th,width:240}}>GL Account</th><th style={{...S.th,width:160}}>Memo</th><th style={{...S.th,width:70}}>Status</th><th style={{...S.th,width:36}}></th></tr></thead>
        <tbody>{txns.map(t=><tr key={t.id} style={t.status==='posted'?{opacity:0.45}:{}}>
          <td style={{...S.td,color:T.textMuted,fontSize:12,whiteSpace:'nowrap'}}>{t.date}</td>
          <td style={{...S.td,maxWidth:240,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:T.textBright}}>{t.description}</td>
          ...
          <td style={S.td}>{t.status!=='posted'&&<button style={S.btnGhost} onClick={async()=>{await api.deleteBankTransaction(entityId,t.id);load();}}>x</button>}</td>
        </tr>)}</tbody></table></div>}
    {selAcct&&txns.length===0&&<div style={{...S.card,textAlign:'center',padding:60,color:T.textDim}}>No transactions. Upload a bank statement to get started.</div>}
...
function TrialBalance({entityId}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const fyStart=asOf.slice(0,4)+'-01-01';
  ...
  return(<div><div style={S.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    ...
    <div style={S.reportHeader}><div style={{fontSize:20,fontWeight:700,color:T.textBright}}>Trial Balance</div><div style={{fontSize:13,color:T.textMuted}}>As of {asOf}</div></div>
...
function BalanceSheet({entityId}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const fyStart=asOf.slice(0,4)+'-01-01';
  ...
  const Sec=({title,type,total})=>(<><tr><td style={S.sectionHeader} colSpan={2}>{title}</td></tr>{get(type).map(b=><tr key={b.code}><td style={S.indentTd}>{b.name}</td><td style={{...S.tdR,borderBottom:'1px solid '+T.border+'40'}}>{fmt(b.balance)}</td></tr>)}
  ...
  return(<div><div style={S.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    <div style={S.filterBar}><div><label style={S.label}>As of Date</label><input style={S.inputSm} type="date" value={asOf} onChange={e=>setAsOf(e.target.value)}/></div></div>
    <button style={S.btnExport} onClick={doExport}>Export Excel</button></div>
    <div style={S.reportHeader}><div style={{fontSize:20,fontWeight:700,color:T.textBright}}>Balance Sheet</div><div style={{fontSize:13,color:T.textMuted}}>As of {asOf}</div></div>
...
function IncomeStatement({entityId}){const[balances,setBalances]=useState([]);const[from,setFrom]=useState(fy_start());const[to,setTo]=useState(today());
  ...
  const doExport=()=>{const d=[['Income Statement'],['Period: '+from+' to '+to],[]];[['Revenue',rev],['COGS',cogs],['Operating Expenses',opex],['Other',other]].forEach(([t,items])=>{if(!items.length)return;d.push([t,'']);items.forEach(b=>d.push(['  '+b.name,b.balance]));d.push(['Total '+t,sum(items)]);d.push([]);});d.push(['Net Income',ni]);exportToExcel(d,'IS_'+from+'_'+to+'.xlsx');};
  const Sec=({title,items,total})=>(<><tr><td style={S.sectionHeader} colSpan={2}>{title}</td></tr>{items.map(b=><tr key={b.code}><td style={S.indentTd}>{b.name}</td><td style={{...S.tdR,borderBottom:'1px solid '+T.border+'40'}}>{fmt(b.balance)}</td></tr>)}
    <tr style={S.subtotalRow}><td style={{...S.td,fontWeight:600,paddingLeft:14}}>Total {title}</td><td style={{...S.tdR,fontWeight:700,color:T.textBright}}>${fmt(total)}</td></tr></>);
  return(<div><div style={S.card}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    <div style={S.filterBar}><div><label style={S.label}>From</label><input style={S.inputSm} type="date" value={from} onChange={e=>setFrom(e.target.value)}/></div>
      <div><label style={S.label}>To</label><input style={S.inputSm} type="date" value={to} onChange={e=>setTo(e.target.value)}/></div></div>
    <button style={S.btnExport} onClick={doExport}>Export Excel</button></div>
    <div style={S.reportHeader}><div style={{fontSize:20,fontWeight:700,color:T.textBright}}>Income Statement</div><div style={{fontSize:13,color:T.textMuted}}>Period: {from} to {to}</div></div>
...
function BankReconciliation({entityId,user}){const[accounts,setAccounts]=useState([]);const[entries,setEntries]=useState([]);const[recs,setRecs]=useState([]);
  ...
  if(view==='new')return(<div><button style={{...S.btnS,marginBottom:20}} onClick={()=>{setView('list');setSelAcct('');setChecked({});}}>&larr; Back to Reconciliations</button>
    ...
    {selAcct&&<><div style={S.summaryBar}>
      {[{l:'Book Balance',v:bookBal,c:T.textBright},{l:'Statement',v:stmtNum,c:T.textBright},{l:'Outstanding Deposits',v:outDep,c:T.green},{l:'Outstanding Payments',v:outPay,c:T.red},{l:'Adjusted Bank',v:stmtNum+outDep+outPay,c:T.accent},{l:'Difference',v:diff,c:isRec?T.green:T.red}].map(s=>
        <div key={s.l} style={{...S.summaryItem,border:s.l==='Difference'&&isRec?'1px solid '+T.greenBorder:undefined,background:s.l==='Difference'&&isRec?T.greenDim:undefined}}>
          <div style={{...S.statVal,fontSize:18,color:s.c}}>${fmt(s.v)}</div><div style={S.statLabel}>{s.l}</div>
          {s.l==='Difference'&&isRec&&<div style={{fontSize:10,color:T.green,marginTop:4,fontWeight:600}}>Reconciled</div>}</div>)}</div>
      <div style={S.cardFlush}><div style={{padding:'14px 20px',display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid '+T.border}}>
        <div style={S.h2}>Uncleared Transactions ({uncl.length})</div>
        <div style={{display:'flex',gap:8}}><button style={{...S.btnS,padding:'6px 14px',fontSize:11}} onClick={()=>{const nc={};uncl.forEach(t=>{nc[t.key]=true;});setChecked(nc);}}>Select All</button>
          <button style={{...S.btnS,padding:'6px 14px',fontSize:11}} onClick={()=>setChecked({})}>Clear All</button></div></div>
        {uncl.length===0?<div style={{padding:30,textAlign:'center',color:T.textDim}}>All transactions cleared</div>:
        <table style={S.table}><thead><tr><th style={S.thC} width={40}>Clr</th><th style={S.th}>Date</th><th style={S.th}>JE</th><th style={S.th}>Memo</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th><th style={S.thR}>Amount</th></tr></thead>
          <tbody>{uncl.map(t=><tr key={t.key} style={{cursor:'pointer',background:checked[t.key]?T.greenDim:'transparent',transition:'background 0.1s'}} onClick={()=>setChecked(p=>({...p,[t.key]:!p[t.key]}))}>
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>Bank Reconciliation</div><div style={{fontSize:13,color:T.textMuted}}>{recs.length} completed reconciliations</div></div>
    <button style={S.btnP} onClick={()=>setView('new')}>+ New Reconciliation</button></div>
    ...
    <div style={S.cardFlush}><div style={{padding:'16px 20px'}}><div style={S.h2}>Reconciliation History</div></div>
      {recs.length===0?<div style={{padding:40,textAlign:'center',color:T.textDim}}>No reconciliations yet</div>:
...
function EntityManagement({refresh,entities,activeEntity,setActiveEntity}){const[showAdd,setShowAdd]=useState(false);const[bulk,setBulk]=useState(false);const[form,setForm]=useState({code:'',name:''});const[bulkText,setBulkText]=useState('');const[err,setErr]=useState('');
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>Entity Management</div><div style={{fontSize:13,color:T.textMuted}}>{entities.length} entities</div></div>
    ...
    {bulk&&<div style={{...S.card,borderColor:T.accent+'40'}}><div style={{...S.h2,marginBottom:8}}>Bulk Import</div><div style={{fontSize:12,color:T.textMuted,marginBottom:10}}>One per line: CODE, Entity Name</div>
      <textarea style={{...S.input,height:160,fontFamily:'monospace',fontSize:12,resize:'vertical'}} placeholder={"CLR-F1, County Line Rail Fund I\nSRN, Sabine River & Northern"} value={bulkText} onChange={e=>setBulkText(e.target.value)}/>
      {err&&<div style={S.err}>{err}</div>}<button style={{...S.btnP,marginTop:10}} onClick={async()=>{const ents=bulkText.split('\n').map(l=>l.trim()).filter(Boolean).map(l=>{const[c,...r]=l.split(',').map(p=>p.trim());return{code:c,name:r.join(',')};}).filter(e=>e.code&&e.name);if(!ents.length){setErr('No valid entries');return;}try{await api.bulkCreateEntities(ents);setBulkText('');setBulk(false);refresh();}catch(e){setErr(e.message);}}}>Import</button></div>}
    <div style={S.cardFlush}><table style={S.table}><thead><tr><th style={S.th}>Code</th><th style={S.th}>Entity</th><th style={{...S.th,width:140}}>Actions</th></tr></thead>
      <tbody>{entities.sort((a,b)=>a.code.localeCompare(b.code)).map(e=><tr key={e.id} style={e.id===activeEntity?{background:T.accentDim}:{}}><td style={{...S.td,fontWeight:700,color:T.accent}}>{e.code}</td><td style={{...S.td,color:T.textBright}}>{e.name}</td>
        <td style={S.td}><div style={{display:'flex',gap:8}}><button style={{...S.btnS,padding:'5px 12px',fontSize:11}} onClick={()=>setActiveEntity(e.id)}>Select</button><button style={{...S.btnD,padding:'5px 12px',fontSize:11}} onClick={async()=>{if(!confirm('Delete this entity?'))return;await api.deleteEntity(e.id);const r=await refresh();if(activeEntity===e.id)setActiveEntity(r[0]?.id||null);}}>Delete</button></div></td></tr>)}</tbody></table></div></div>);}
...
function UserManagement({currentUser}){const[users,setUsers]=useState([]);const[showAdd,setShowAdd]=useState(false);const[form,setForm]=useState({name:'',email:'',password:'',role:'Viewer'});const[err,setErr]=useState('');
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    <div><div style={S.h1}>User Management</div><div style={{fontSize:13,color:T.textMuted}}>{users.length} users</div></div>
    ...
    {showAdd&&<div style={{...S.card,borderColor:T.green+'40'}}><div style={S.row}>
      ...
      {err&&<div style={S.err}>{err}</div>}<button style={S.btnP} onClick={async()=>{if(!form.name||!form.email||!form.password){setErr('All fields required');return;}try{await api.signup(form.name,form.email,form.password,form.role);setForm({name:'',email:'',password:'',role:'Viewer'});setShowAdd(false);setErr('');api.getUsers().then(setUsers);}catch(e){setErr(e.message);}}}>Add User</button></div>}
    <div style={S.cardFlush}><table style={S.table}><thead><tr><th style={S.th}>Name</th><th style={S.th}>Email</th><th style={S.th}>Role</th><th style={{...S.th,width:200}}>Actions</th></tr></thead>
      <tbody>{users.map(u=><tr key={u.id}><td style={{...S.td,fontWeight:600,color:T.textBright}}>{u.name}{u.id===currentUser.id?<span style={{color:T.accent,fontSize:10,marginLeft:8}}>(you)</span>:''}</td><td style={S.td}>{u.email}</td><td style={S.td}><span style={S.badge}>{u.role}</span></td>
        <td style={S.td}><div style={{display:'flex',gap:8}}>{u.id!==currentUser.id&&<button style={{...S.btnS,padding:'5px 12px',fontSize:11}} onClick={()=>{setResetId(u.id);setResetPw('');setResetMsg('');}}>Reset PW</button>}
          {u.id!==currentUser.id&&<button style={{...S.btnD,padding:'5px 12px',fontSize:11}} onClick={async()=>{await api.deleteUser(u.id);setUsers(p=>p.filter(x=>x.id!==u.id));}}>Delete</button>}</div></td></tr>)}</tbody></table></div>
    {resetId&&<div style={S.modal} onClick={()=>setResetId(null)}><div style={{...S.modalBox,maxWidth:400,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{fontSize:13,color:T.textMuted,marginBottom:16}}>For: {users.find(u=>u.id===resetId)?.email}</div>
