...
function Logo({size=32}) {
  return (<svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M10 14h20M10 20h14M10 26h18" stroke="#fff" strokeWidth="2.4" strokeLinecap="round"/>
    <path d="M28 17v10l4-5-4-5z" fill="#fff" opacity="0.7"/>
    <circle cx="30" cy="13" r="3" fill="#34d399"/>
    <defs><linearGradient id="lg" x1="0" y1="0" x2="40" y2="40" gradientUnits="userSpaceOnUse">
      <stop stopColor="#2563eb"/><stop offset="1" stopColor="#059669"/></linearGradient></defs>
  </svg>);
}

// Auto-categorization hints
const HINTS = [
  { kw:['payroll','salary','wage','bonus'], sub:['Operating Expense'], re:/salari|payroll/i },
  { kw:['rent','lease'], sub:['Operating Expense'], re:/rent/i },
  { kw:['electric','gas','water','utility','utilities'], sub:['Operating Expense'], re:/utilit/i },
  { kw:['insurance','premium'], sub:['Operating Expense'], re:/insurance/i },
  { kw:['supplies','office depot','staples','amazon'], sub:['Operating Expense'], re:/supplies|office/i },
  { kw:['advertising','marketing','google ads','facebook','ad spend'], sub:['Operating Expense'], re:/market/i },
  { kw:['interest','loan payment','finance charge'], sub:['Other Expense'], re:/interest/i },
  { kw:['deposit','payment received','wire in','ach credit','revenue','sales'], sub:['Operating Revenue','Other Revenue'], re:/revenue|income/i },
];
function suggestAccount(desc, accounts, bankCode) {
  if (!desc) return null; const d = desc.toLowerCase();
  for (const h of HINTS) { if (h.kw.some(k => d.includes(k))) { const cs = accounts.filter(a => a.code !== bankCode && h.sub.includes(a.subtype)); return cs.find(a => h.re.test(a.name)) || cs[0] || null; } }
  return null;
}
...
function JournalEntryModal({entityId,user,onClose,onPosted,form,setForm,pendingFiles,setPendingFiles}){
  ...
  const post=async()=>{if(!form.date||!form.memo.trim()){setErr('Date and memo required');return;}if(form.lines.some(l=>!l.account_code)){setErr('All lines need an account');return;}if(!bal){setErr('Entry must balance');return;}
    setPosting(true);setErr('');try{const r=await api.createEntry(entityId,{date:form.date,memo:form.memo.trim(),lines:form.lines.map(l=>({account_code:l.account_code,debit:parseFloat(l.debit)||0,credit:parseFloat(l.credit)||0}))});
      ...
      if(pendingFiles.length>0){try{const u=await api.uploadAttachments(entityId,r.id,pendingFiles);msg+=' with '+u.length+' attachment(s)';}catch(ue){msg+=' (attachments failed)';}}
  ...
  return(<div style={S.modal}><div style={{...S.modalBox,maxWidth:980}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{border:'1px solid '+(pendingFiles.length>0?T.teal+'40':T.border),borderRadius:T.radiusSm,padding:16,marginBottom:16,background:pendingFiles.length>0?T.tealDim:'#fff'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:pendingFiles.length>0?12:0}}>
        ...
        <div><input type="file" multiple accept=".pdf,.xlsx,.xls,.csv,.png,.jpg,.jpeg,.eml,.msg,.doc,.docx" style={{display:'none'}} id="je-file-input" onChange={e=>{if(e.target.files.length>0)setPendingFiles(p=>[...p,...Array.from(e.target.files)]);e.target.value='';}}/><label htmlFor="je-file-input" style={{...S.btnS,padding:'7px 16px',cursor:'pointer',display:'inline-block'}}>Attach files</label></div></div>
...
function BankTransactions({entityId}){const[accounts,setAccounts]=useState([]);const[bankAccts,setBankAccts]=useState([]);const[txns,setTxns]=useState([]);
  ...
  const handleUpload=async e=>{const file=e.target.files[0];if(!file||!selAcct)return;e.target.value='';setErr('');setMsg('');setUploading(true);setUploadProgress('Uploading file...');
    try{const r=await api.uploadBankTransactions(entityId,selAcct,file);
      ...
      setMsg(r.count+' imported'+(auto>0?', '+auto+' auto-categorized':''));load();}catch(ex){setErr(ex.message);}finally{setUploading(false);setUploadProgress('');}};
  ...
  const codeTransaction=async(id,acct_code,memo)=>{await api.codeBankTransaction(entityId,id,acct_code,memo);
    // Update local state instead of full reload so coded accounts stay visible
    setTxns(prev=>prev.map(t=>t.id===id?{...t,account_code:acct_code,memo:memo,status:acct_code?'coded':'pending'}:t));};
  const postCoded=async()=>{const ids=txns.filter(t=>t.status==='coded').map(t=>t.id);if(!ids.length){setErr('Nothing coded');return;}try{const r=await api.postBankTransactions(entityId,ids);setMsg(r.posted+' JEs created');load();}catch(ex){setErr(ex.message);}};
