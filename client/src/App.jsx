...
function BankTransactions({entityId,bankSelAcct:selAcct,setBankSelAcct:setSelAcct,bankTxns:txns,setBankTxns:setTxns,bankUploading:uploading,setBankUploading:setUploading,bankStatusFilter:statusFilter,setBankStatusFilter:setStatusFilter}){
  ...
  const resizeHandle=key=><span onMouseDown={ev=>startResize(key,ev)} style={{position:'absolute',right:0,top:0,bottom:0,width:6,cursor:'col-resize',userSelect:'none'}} title="Drag to resize column"/>;
  ...
  return(<div><div style={S.h1}>Bank Transactions</div><div style={S.sub}>Upload, categorize, and post bank activity to the general ledger</div>
    ...
    {selAcct&&filteredTxns.length>0&&<div style={S.cardFlush}>
      ...
      <table style={{...S.table,tableLayout:'fixed',width:'100%'}}>
        ...
        <thead><tr>
          <th style={{...S.th,position:'relative'}}>Date{resizeHandle('date')}</th>
          <th style={{...S.th,position:'relative'}}>Description{resizeHandle('desc')}</th>
          <th style={{...S.thR,position:'relative'}}>Amount{resizeHandle('amount')}</th>
          <th style={{...S.th,position:'relative'}}>GL Account{resizeHandle('gl')}</th>
          <th style={{...S.th,position:'relative'}}>Memo{resizeHandle('memo')}</th>
          <th style={{...S.th,position:'relative'}}>Status{resizeHandle('status')}</th>
          <th style={{...S.th,width:36}}></th></tr></thead>
        <tbody>{filteredTxns.map(t=><tr key={t.id} style={t.status==='posted'?{opacity:0.45}:{}}>
          <td style={{...S.td,color:T.textMuted,fontSize:12}} title={t.date}>{t.date}</td>
          <td style={{...S.td,fontWeight:500}} title={t.description}>{t.description}</td>
          <td style={{...S.tdR,fontSize:15,fontWeight:700,color:t.amount>=0?T.green:T.red}}>{t.amount>=0?'+':''}{fmt(t.amount)}</td>
          <td style={{...S.td,padding:'4px 6px',overflow:'visible'}}>{t.status==='posted'
          ...
          <td style={{...S.td,padding:'4px 6px',overflow:'visible'}}>{t.status==='posted'?<span style={{fontSize:12,color:T.textDim}}>{t.memo}</span>:
            (t.splits && t.splits.length>0
