...
export default function App(){
  ...
  const navTree=[
    ...
    {key:'REPORTS',label:'Reports',icon:'📊',items:[
      ...
      ...(!isCLRF?[{id:'b2a',label:'Operating Budget to Actual',icon:'🎯',section:'reports'}]:[]),
...
function GeneralLedger({entityId,entityName,dimsEnabled,from,setFrom,to,setTo,filter,setFilter}){const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);
  ...
  const doExport=()=>{const rows=[[entityName||'General Ledger'],['General Ledger'],['Period: '+(from||'Begin')+' to '+(to||today())],[]];const F=[];filtered.forEach(acct=>{const txns=[];entries.forEach(e=>{e.lines.forEach(l=>{if(l.account_code===acct.code&&(!projFilter||Number(l.project_id)===Number(projFilter)))txns.push({date:e.date,je:'JE-'+String(e.entry_num).padStart(4,'0'),doc:e.doc_number||'',vendor:e.vendor||'',memo:l.description||e.memo,debit:l.debit,credit:l.credit});});});if(txns.length===0&&!filter)return;rows.push([acctLabel(acct.code,acct.name)]);rows.push(['Date','JE','Doc #','Vendor','Memo','Debit','Credit','Balance']);const isDr=acct.type==='Asset'||acct.type==='Expense';let run=from?(Number(begBals[acct.code])||0):0;let prevRow=null;if(from){rows.push(['','','','','Beginning Balance','','',run]);prevRow=rows.length-1;}txns.sort((a,b)=>a.date.localeCompare(b.date)).forEach(t=>{const _db=Number(t.debit)||0,_cr=Number(t.credit)||0;run+=isDr?(_db-_cr):(_cr-_db);rows.push([t.date,t.je,t.doc,t.vendor,t.memo,_db||'',_cr||'',run]);const r=rows.length-1,rr=r+1;const delta=isDr?('F'+rr+'-G'+rr):('G'+rr+'-F'+rr);F.push({r,c:7,f:prevRow!=null?('H'+(prevRow+1)+'+'+delta):delta});prevRow=r;});rows.push([]);});exportToExcel(rows,'GL.xlsx',{formulas:F});};
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}><div><div style={S.h1}>General Ledger</div>{entityName&&<div style={{fontSize:13,color:T.textMuted}}>{entityName}</div>}</div><button style={S.btnExport} onClick={doExport}>Export Excel</button></div><div style={S.sub}/>
    ...
    {filtered.map(acct=>{const txns=[];entries.forEach(e=>{e.lines.forEach(l=>{if(l.account_code===acct.code&&(!projFilter||Number(l.project_id)===Number(projFilter)))txns.push({...l,date:e.date,memo:e.memo,doc:e.doc_number||'',vendor:e.vendor||'',jeNum:e.entry_num,jeId:e.id});});});
      ...
      return(<div key={acct.code} style={S.cardFlush}><div style={{padding:'14px 20px',display:'flex',alignItems:'center',gap:10,borderBottom:'1px solid '+T.border}}>
        ...
        <div style={{overflowX:'auto'}}><table style={{...S.table,minWidth:1100}}><thead><tr><th style={S.th}>Date</th><th style={S.th}>JE</th><th style={S.th}>Doc #</th><th style={S.th}>Vendor</th><th style={S.th}>Memo</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th><th style={S.thR}>Balance</th><th style={{...S.th,width:100}}>Docs</th></tr></thead>
          <tbody>{from&&<tr><td style={{...S.td,color:T.textMuted}}>{from}</td><td style={S.td}></td><td style={S.td}></td><td style={S.td}></td><td style={{...S.td,fontStyle:'italic',color:T.textMuted}}>Beginning Balance</td><td style={S.tdR}></td><td style={S.tdR}></td><td style={{...S.tdR,fontWeight:600,color:T.textMuted}}>{fmt(beg)}</td><td style={S.td}></td></tr>}{txns.map((t,i)=>{run+=dr?(t.debit-t.credit):(t.credit-t.debit);const atts=entryAtts[t.jeId];return<tr key={i}><td style={{...S.td,color:T.textMuted}}>{t.date}</td><td style={S.td}><button style={{background:'none',border:0,padding:0,color:T.accent,fontWeight:600,cursor:'pointer',fontSize:'inherit',fontFamily:'inherit'}} onClick={()=>{const e=entries.find(x=>x.id===t.jeId);if(e)setEditEntry(e);}}>JE-{String(t.jeNum).padStart(4,'0')}</button></td><td style={{...S.td,color:T.textMuted,whiteSpace:'nowrap'}}>{t.doc||''}</td><td style={S.td} title={t.vendor||''}>{t.vendor||''}</td><td style={S.td} title={t.description||t.memo}>{t.description||t.memo}</td><td style={S.tdR}>{t.debit>0?fmt(t.debit):''}</td><td style={S.tdR}>{t.credit>0?fmt(t.credit):''}</td><td style={{...S.tdR,fontWeight:600,color:T.textBright}}>{fmt(run)}</td>
...
function TrialBalance({entityId,entityName,dimsEnabled,isClrf,asOf,setAsOf,canEdit=true}){
  ...
  const doExportGL=async()=>{
    try{
      ...
      const first=d.length;
      _ln.forEach(l=>d.push([l.date,l.entry_num,l.doc_number||'',l.account_code,l.account_name,l.description||l.memo||'',..._dimOf(l),Number(l.debit)||'',Number(l.credit)||'',Number(l.running_balance)||0]));
      const last=d.length-1;
...
function AccountDrillDownModal({entityId,entityName,acct,from:fromProp,to:toProp,onClose,onChanged}){
  ...
  const doExport=()=>{const acctLabel=acct.code+' - '+acct.name;
    ...
    const STY={titleRows:[0,1],metaRows:[2],headerRows:[4],underlineRows:[],doubleUnderlineRows:[],amountCols:[cDr,cCr,cBal]};
    const F=[];
    d.push([...bl(4+nD),'Beginning Balance',...bl(4),Number(begBal)||0]);let prevRow=d.length-1;const begRow=prevRow;
    let r=Number(begBal)||0;const first=d.length;lines.forEach(l=>{const _db=Number(l.debit)||0,_cr=Number(l.credit)||0;r+=isDr?(_db-_cr):(_cr-_db);d.push([l.date,'JE-'+String(l.entry_num).padStart(4,'0'),l.doc_number||'',acctLabel,...dimOf(l),l.description||l.memo,l.offset||'',l.vendor||'',_db||'',_cr||'',r]);const cur=d.length-1,rr=cur+1;const delta=isDr?(LDr+rr+'-'+LCr+rr):(LCr+rr+'-'+LDr+rr);F.push({r:cur,c:cBal,f:LBal+(prevRow+1)+'+'+delta});prevRow=cur;});const last=d.length-1;
    STY.underlineRows.push(last);
  ...
  return(<div><div style={S.card}>
    ...
    {result&&<div style={{...S.card,marginTop:18,padding:14,background:'#f3faf5'}}>
      ...
      <table style={{...S.table,minWidth:360,marginBottom:10}}><tbody>
        ...
        {kind==='subclose'&&<>
          <tr><td style={S.td}>Investors</td><td style={S.tdR}>{s.investors}</td></tr>
          <tr><td style={S.td}>Equalization cash to/(from) investors</td><td style={S.tdR}>{fmt(s.equalization_cash)}</td></tr>
          <tr style={S.grandTotalRow}><td style={S.tdBold}>Ending capital (contributed)</td>
            <td style={{...S.tdBold,textAlign:'right'}}>{fmt(s.ending_capital)}</td></tr>
        </>}
        {kind==='cashflow'&&<>
...
function Requisitions({entityId,entityName,canEdit=true,reqState,setReqState}){
  ...
  const[draftLoaded,setDraftLoaded]=useState(false);// false until loadDraft('') finishes, so the first-time card never flashes before data arrives
  ...
  const[nextAsOfPhase,setNextAsOfPhase]=useState(null);// which finalized phase's 'start next' as-of prompt is open (null=closed)
  const[nextAsOf,setNextAsOf]=useState('');// the as-of date entered for the next requisition
  ...
  const[manualOverwrite,setManualOverwrite]=useState(null);// {fileName} — Drafts-folder workbook edited by hand// {message, workbookFile, reqNumber, asOfDate}
  const loadDraft=async(phaseArg)=>{const wantPhase=phaseArg!==undefined?phaseArg:activePhase;let rail=false;let openList=[];let finList=[];try{const r=await api.getRequisitionDraft(entityId,undefined);rail=!!(r&&r.is_rail);setIsRail(rail);openList=(r&&r.drafts)||[];finList=(r&&r.finalized)||[];setDraftList(openList);setFinalizedList(finList);let sel=null;if(wantPhase!==undefined&&wantPhase!==null&&wantPhase!==''){sel=openList.find(d=>(d.phase||'')===(wantPhase||''))||null;}if(!sel){sel=openList.find(d=>(d.phase||'')===(activePhase||''))||openList[0]||null;}const _loadedDraft=sel;setDraft(_loadedDraft);if(_loadedDraft)setActivePhase(_loadedDraft.phase||'');else if(wantPhase!==undefined&&wantPhase!==null)setActivePhase(wantPhase||'');if(_loadedDraft){setReqState(cur=>({...cur,asOf:_loadedDraft.as_of_date||cur.asOf||'',reqNum:(_loadedDraft.req_number!=null?String(_loadedDraft.req_number):cur.reqNum)}));}}catch(e){setDraft(null);}try{const s=await api.getRequisitionSeedSource(entityId,(wantPhase&&wantPhase!=='')?wantPhase:undefined);setSeedSource(s?s.source:null);if(s&&typeof s.is_rail==='boolean'){rail=s.is_rail;setIsRail(s.is_rail);}}catch{setSeedSource(null);}const nothingYet=openList.length===0&&finList.length===0;if(nothingYet){setFtRows(rail?[{phase:'',file:null,reqNum:''},{phase:'',file:null,reqNum:''}]:[{phase:'',file:null,reqNum:''}]);}setDraftLoaded(true);};
  ...
  const rollDraft=async(force)=>{setDraftBusy(true);setDraftErr('');setDraftMsg('');if(force)setManualOverwrite(null);try{const r=await api.rollRequisitionDraft(entityId,{reqNumber:rfReqNum||undefined,asOfDate:rfAsOf||undefined,phase:activePhase||undefined,force:!!force});setDraft(r&&r.draft);setDraftMsg(r&&r.ok?'Saved — reconciled.':'Saved — needs review (reconciliation not balanced).');}catch(e){if(e.detail&&e.detail.manualEdit){setManualOverwrite({fileName:e.detail.fileName||'the workbook in the Drafts folder'});}else{setDraftErr(e.message);}}finally{setDraftBusy(false);}};
  ...
  const createAddedPhase=async()=>{setApErr('');if(!apRow.file){setApErr('Choose the prior workbook for this phase.');return;}if(!apRow.asOf){setApErr('Enter the as-of date.');return;}const ph=reqDraftNormPhase(apRow.phase);if(isRail&&!ph){setApErr('Enter the phase # (e.g. 2a).');return;}
    ...
    setApBusy(true);try{await api.createRequisitionDraft(entityId,{workbookFile:apRow.file,reqNumber:apRow.reqNum||undefined,asOfDate:apRow.asOf,phase:ph||undefined});setAddingPhase(false);setActivePhase(ph||'');setFinConfirm(false);setDraftMsg('Phase '+ph+' draft created.');await loadDraft(ph||'');}catch(e){setApErr(e.message);}finally{setApBusy(false);}};
  const startNext=async(phase,asOf)=>{setDraftBusy(true);setDraftErr('');setDraftMsg('');setFinConfirm(false);try{const r=await api.createRequisitionDraft(entityId,{phase:phase||undefined,asOfDate:asOf||undefined});setActivePhase(phase||'');setDraft(r&&r.draft);setNextAsOfPhase(null);setNextAsOf('');setDraftMsg('Next requisition started \u2014 seeded from the filed report. Add this period\u2019s invoices, then Prepare.');await loadDraft(phase||'');}catch(e){setDraftErr(e.message);}finally{setDraftBusy(false);}};
  const reopenDraft=async(phase)=>{setDraftBusy(true);setDraftErr('');setDraftMsg('');setFinConfirm(false);try{const r=await api.reopenRequisitionDraft(entityId,phase||undefined);setActivePhase(phase||'');setDraft(r&&r.draft);setDraftMsg('Reopened for edits — change invoices and re-finalize.');await loadDraft(phase||'');}catch(e){setDraftErr(e.message);}finally{setDraftBusy(false);}};
  ...
  const discardFinalized=async(phase)=>{if(!confirm('Discard this finalized requisition and start this entity over? The requisition record is removed so you return to the first-time upload screen. This cannot be undone.'))return;setDraftBusy(true);setDraftErr('');setDraftMsg('');setFinConfirm(false);try{await api.reopenRequisitionDraft(entityId,phase||undefined);await api.discardRequisitionDraft(entityId,phase||undefined);setDraftMsg('Requisition discarded — back to first-time setup.');setActivePhase('');await loadDraft('');}catch(e){setDraftErr(e.message);}finally{setDraftBusy(false);}};
  ...
  return(<div>
    ...
    {!draftLoaded&&!draft&&<div style={{fontSize:13,color:T.textMuted,padding:'10px 2px'}}>Loading\u2026</div>}
    {draftLoaded&&draftList.length===0&&finalizedList.length===0&&<div style={{...S.card,marginBottom:16,borderLeft:'4px solid '+T.accent}}>
    ...
    {(()=>{const fin=finalizedList.find(f=>(f.phase||'')===(activePhase||''))||(draftList.length===0?finalizedList[0]:null);if(!fin||draft)return null;return(
      <div style={{...S.card,marginBottom:16,borderLeft:'4px solid '+T.green}}>
        ...
        {nextAsOfPhase===(fin.phase||'')&&<div style={{marginTop:12,padding:14,background:T.bgElevated,borderRadius:8,border:'1px solid '+T.border}}>
          ...
          <div style={{fontSize:12,color:T.textMuted,marginBottom:10}}>Enter the period-end (as-of) date for this requisition report. This date is used in the report and its filename.</div>
          <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
            <input type="date" style={S.inputSm} value={nextAsOf} onChange={e=>setNextAsOf(e.target.value)}/>
            <button style={{...S.btnP}} disabled={draftBusy||!nextAsOf} onClick={()=>startNext(fin.phase||'',nextAsOf)}>{draftBusy?'Starting\u2026':'Start requisition'}</button>
            <button style={{...S.btnS,color:T.textMuted}} disabled={draftBusy} onClick={()=>{setNextAsOfPhase(null);setNextAsOf('');}}>Cancel</button>
          </div>
        </div>}
      </div>);})()}
    {draft&&<div style={{...S.card,marginBottom:16,borderLeft:'4px solid '+(draft.recon_ok===false?T.orange:(draft.recon_ok?T.green:T.accent))}}>
      ...
      {showReview&&draft.recon_ok===false&&(()=>{const rc=draft.recon||{};const failed=(rc.checks||[]).filter(c=>!c.pass);return(
      ...
      {manualOverwrite&&<div style={{marginTop:12,padding:14,background:T.orange+'12',borderRadius:8,border:'1px solid '+T.orange+'55'}}>
        <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:4}}>This draft has been updated by hand</div>
        <div style={{fontSize:12,color:T.textMuted,marginBottom:10}}>{manualOverwrite.fileName} in the Drafts folder no longer matches what CloudLedger last built — someone updated it in Excel. Preparing again rebuilds the workbook from the invoices and those manual edits are lost. If the workbook is already right, skip Prepare and go straight to Finalize: the version in the Drafts folder is what gets filed.</div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <button style={{...S.btnS,color:T.red,borderColor:T.red+'55'}} disabled={draftBusy} onClick={()=>rollDraft(true)}>Rebuild anyway (discards the manual edits)</button>
          <button style={S.btnGhost} disabled={draftBusy} onClick={()=>setManualOverwrite(null)}>Keep the manual version</button>
        </div>
      </div>}
      {finConfirm&&draft.has_output&&<div style={{marginTop:12,padding:14,background:T.bgElevated,borderRadius:8,border:'1px solid '+T.border}}>
...
function EntityManagement({refresh,entities,activeEntity,setActiveEntity}){
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
    ...
    {bulk&&(()=>{const TYPE_ALIASES={accounting:'accounting',acct:'accounting',acc:'accounting',development:'development','development project':'development',dev:'development',devproject:'development',shell:'shell','rail assets':'rail_assets',rail:'rail_assets',railassets:'rail_assets',rail_assets:'rail_assets'};
