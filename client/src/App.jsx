...
export default function App(){
  ...
  return(<div style={S.app}>
    <div style={S.topBar}><div style={{display:'flex',alignItems:'center',gap:16}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}><div style={S.logoIcon}>GL</div>{!sidebarCol&&<div style={{fontSize:17,fontWeight:800,color:T.textBright}}>CloudLedger</div>}</div>
    ...
    {entries.length===0?<div style={{...S.card,textAlign:'center',padding:60,color:T.textDim}}>No entries found</div>:
      entries.map(e=><div key={e.id} style={{...S.card,padding:18}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:12,color:T.textDim}}>{e.created_by}</span><button style={S.btnD} onClick={()=>del(e.id)}>Delete</button></div></div>
...
function BankTransactions({entityId}){const[accounts,setAccounts]=useState([]);const[bankAccts,setBankAccts]=useState([]);const[txns,setTxns]=useState([]);
  ...
  const handleUpload=async e=>{const file=e.target.files[0];if(!file||!selAcct)return;e.target.value='';setErr('');setMsg('');setUploading(true);
    try{const r=await api.uploadBankTransactions(entityId,selAcct,file);
      ...
      setMsg(r.count+' imported'+(auto>0?', '+auto+' auto-categorized':''));load();}catch(ex){setErr(ex.message);}finally{setUploading(false);}};
  const cancelUpload=()=>{setUploading(false);setMsg('Cancelled');};
  const codeTransaction=async(id,acct_code,memo)=>{await api.codeBankTransaction(entityId,id,acct_code,memo);load();};
  ...
  return(<div><div style={S.h1}>Bank Transactions</div><div style={S.sub}>Upload, categorize, and post bank activity to the general ledger</div>
    <div style={S.card}><div style={S.row}>
      ...
      {selAcct&&<div style={{display:'flex',gap:8,alignItems:'flex-end'}}>
        {uploading?<button style={{...S.btnD,padding:'9px 20px',fontSize:13}} onClick={cancelUpload}>Cancel Upload</button>
          :<><input type="file" accept=".csv,.xlsx,.xls" id="bank-upload" style={{display:'none'}} onChange={handleUpload}/><label htmlFor="bank-upload" style={{...S.btnP,cursor:'pointer',display:'inline-block'}}>{uploading?'Uploading...':'Upload CSV / Excel'}</label></>}
      </div>}
