...
export const api = {
  ...
  updateArInvoiceDetails: (eid, id, body = {}) => request('/entities/' + eid + '/ar/invoices/' + id + '/details', { method: 'PATCH', body }),
  ...
  unsyncBillcom: (entityId, dryRun, asOf) => request('/billcom/unsync/' + entityId, { method: 'POST', body: { dry_run: !!dryRun, as_of: asOf || null } }),
  ...
  // Month-End Leadsheets — the monthly FloQast-format close package, one
  // workbook per leadsheet, returned as a .zip (copies filed under Workpapers).
  monthEndLeadsheets: async (eid, monthEnd) => {
    const token = getToken();
    const res = await fetch(API_BASE + '/workpapers/leadsheets/' + eid + '/generate', {
      method: 'POST',
      headers: Object.assign({ 'Content-Type': 'application/json' }, token ? { Authorization: 'Bearer ' + token } : {}),
    ...
    let summary = {}; try { summary = JSON.parse(res.headers.get('x-leadsheets-summary') || '{}'); } catch {}
    const cd = res.headers.get('content-disposition') || '';
    const m = cd.match(/filename="?([^"]+)"?/);
    return { blob: await res.blob(), filename: m ? m[1] : 'Month_End_Leadsheets.zip', summary };
  },
