...
function exportToExcel(data, fileName) {
  ...
  ws['!cols'] = data[0].map((_, ci) => ({ wch: Math.min(Math.max(...data.map(r => String(r[ci]||'').length) , 8) + 2, 40) }));
...
function AuthScreen({ onLogin }) {
  const [mode,setMode]=useState('login');const [email,setEmail]=useState('');const [pw,setPw]=useState('');const [name,setName]=useState('');const [confirmPw,setConfirmPw]=useState('');const [role,setRole]=useState('Accountant');
  ...
  const doForgot=async()=>{if(!email.trim()){setErr('Enter email');return;}setLoading(true);setErr('');try{const r=await api.forgotPassword(email.trim().toLowerCase());setTempPw(r.temp_password);setSuccess('Reset!');}catch(e){setErr(e.message);}finally{setLoading(false);}};
  ...
  return(<div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',background:C.bg}}><div style={{...S.card,width:400,textAlign:'center',padding:36}}>
    ...
    {mode==='forgot'?(<>
      <div style={{fontSize:14,fontWeight:600,color:C.textBright,marginBottom:16}}>Reset Password</div>
      ...
      {tempPw&&<div style={{background:C.green+'15',border:'1px solid '+C.green+'40',borderRadius:8,padding:16,margin:'10px 0'}}>
        <div style={{fontSize:12,color:C.green,marginBottom:6}}>Temporary password:</div>
        <div style={{fontSize:18,fontWeight:700,color:C.textBright,fontFamily:'monospace'}}>{tempPw}</div>
        <div style={{fontSize:11,color:C.textDim,marginTop:8}}>Sign in with this, then change in Settings.</div></div>}
      <button style={{...S.btnP,width:'100%',padding:9,marginTop:6}} onClick={doForgot} disabled={loading}>{loading?'...':'Reset Password'}</button>
      ...
      {mode==='login'?(<>
        ...
        {err&&<div style={S.err}>{err}</div>}
        <button style={{...S.btnP,width:'100%',padding:9,marginTop:6}} onClick={doLogin} disabled={loading}>{loading?'...':'Sign In'}</button>
...
function ChangePasswordModal({onClose}){const[cur,setCur]=useState('');const[nw,setNw]=useState('');const[cf,setCf]=useState('');const[err,setErr]=useState('');const[ok,setOk]=useState(false);
  const go=async()=>{if(nw.length<3){setErr('Min 3');return;}if(nw!==cf){setErr("Don't match");return;}try{await api.changePassword(cur,nw);setOk(true);setTimeout(onClose,1500);}catch(e){setErr(e.message);}};
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:380,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{marginBottom:10}}><input style={S.input} type="password" placeholder="Current password" value={cur} onChange={e=>{setCur(e.target.value);setErr('');}}/></div>
    <div style={{marginBottom:10}}><input style={S.input} type="password" placeholder="New password" value={nw} onChange={e=>{setNw(e.target.value);setErr('');}}/></div>
    ...
    <button style={{...S.btnP,width:'100%',padding:9,marginTop:6}} onClick={go}>Update</button>
  </div></div>);}
...
function QuickAddAccountModal({entityId,onClose,onCreated}){const[form,setForm]=useState({code:'',name:'',type:'Asset',subtype:'',bank_acct:false});const[err,setErr]=useState('');
  const go=async()=>{if(!form.code||!form.name){setErr('Required');return;}try{const a=await api.createAccount(entityId,form);onCreated(a);onClose();}catch(e){setErr(e.message);}};
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:600}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={S.row}><div style={S.col}><label style={S.label}>Code</label><input style={S.input} value={form.code} onChange={e=>setForm(f=>({...f,code:e.target.value}))}/></div>
    ...
    {err&&<div style={S.err}>{err}</div>}<div style={{display:'flex',gap:10,marginTop:8}}><button style={S.btnP} onClick={go}>Add</button><button style={S.btnS} onClick={onClose}>Cancel</button></div>
  </div></div>);}

// ─── Floating JE Modal with Attachments ───
function JournalEntryModal({entityId,user,onClose,onPosted}){
  const [accounts,setAccounts]=useState([]);const [showAddAcct,setShowAddAcct]=useState(false);const [pendingFiles,setPendingFiles]=useState([]);
  const blank={date:today(),memo:'',lines:[{account_code:'',debit:'',credit:''},{account_code:'',debit:'',credit:''}]};
  const [form,setForm]=useState(blank);const [err,setErr]=useState('');const [posting,setPosting]=useState(false);const [posted,setPosted]=useState('');
  ...
  const tDr=form.lines.reduce((s,l)=>s+(parseFloat(l.debit)||0),0);const tCr=form.lines.reduce((s,l)=>s+(parseFloat(l.credit)||0),0);
  const bal=Math.abs(tDr-tCr)<0.005&&tDr>0;
  const post=async()=>{
    if(!form.date||!form.memo.trim()){setErr('Date & memo required');return;}if(form.lines.some(l=>!l.account_code)){setErr('All lines need account');return;}if(!bal){setErr('Must balance');return;}
    setPosting(true);setErr('');
    try{const r=await api.createEntry(entityId,{date:form.date,memo:form.memo.trim(),lines:form.lines.map(l=>({account_code:l.account_code,debit:parseFloat(l.debit)||0,credit:parseFloat(l.credit)||0}))});
      if(pendingFiles.length>0){await api.uploadAttachments(entityId,r.id,pendingFiles);}
      setForm(blank);setPendingFiles([]);setPosted('JE-'+String(r.entry_num).padStart(4,'0')+' posted!');setTimeout(()=>setPosted(''),3000);if(onPosted)onPosted();}
    catch(e){setErr(e.message);}finally{setPosting(false);}
  };
  return(<div style={S.modal} onClick={onClose}><div style={{...S.modalBox,maxWidth:950}} onClick={e=>e.stopPropagation()}>
    <button style={S.modalClose} onClick={onClose}>x</button>
    <div style={{fontSize:16,fontWeight:700,color:C.textBright,marginBottom:16}}>New Journal Entry</div>
    <div style={S.row}>
      <div style={{...S.col,maxWidth:160}}><label style={S.label}>Date</label><input style={S.input} type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))}/></div>
      <div style={{...S.col,flex:4}}><label style={S.label}>Memo / Description</label><input style={S.input} placeholder="Enter description" value={form.memo} onChange={e=>setForm(f=>({...f,memo:e.target.value}))}/></div></div>
    <table style={{...S.table,marginBottom:10}}><thead><tr><th style={S.th}>Account</th><th style={{...S.thR,width:130}}>Debit</th><th style={{...S.thR,width:130}}>Credit</th><th style={{...S.th,width:30}}></th></tr></thead>
      <tbody>{form.lines.map((l,i)=><tr key={i}><td style={{padding:'3px 4px',borderBottom:'1px solid '+C.border}}>
        ...
        <td style={{padding:'3px 4px',borderBottom:'1px solid '+C.border,textAlign:'center'}}>{form.lines.length>2&&<span style={{cursor:'pointer',color:C.red}} onClick={()=>removeLine(i)}>x</span>}</td></tr>)}
    ...
    <div style={{marginBottom:12}}>
      <label style={{...S.label,marginBottom:6}}>Attachments (PDF, Excel, images)</label>
      ...
      <input type="file" multiple accept=".pdf,.xlsx,.xls,.csv,.png,.jpg,.jpeg,.eml,.msg,.doc,.docx" style={{display:'none'}} id="je-attach"
    ...
    <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
      ...
      {posted&&<span style={S.success}>{posted}</span>}
  ...
  return(<div style={{position:'relative'}}><div style={{display:'flex',alignItems:'center',gap:6,cursor:'pointer',padding:'5px 12px',borderRadius:6,background:C.border,border:'1px solid '+C.borderLight}} onClick={()=>setOpen(!open)}>
    ...
    {open&&<><div style={{position:'fixed',inset:0,zIndex:50}} onClick={()=>{setOpen(false);setSearch('');}}/>
      <div style={{position:'absolute',top:'100%',left:0,background:C.card,border:'1px solid '+C.border,borderRadius:8,maxHeight:350,overflowY:'auto',zIndex:100,boxShadow:'0 12px 40px rgba(0,0,0,0.5)',width:320}}>
        ...
        <div style={{borderTop:'1px solid '+C.border,padding:10}}><button style={{...S.btnS,width:'100%',fontSize:11}} onClick={()=>{onManage();setOpen(false);}}>Manage Entities</button></div></div></>}
  </div>);}
  ...
  return(<div style={S.app}>
    ...
    <div style={S.body}><div style={S.sidebar}>{navItems.map(n=>n.divider?<div key={n.id} style={S.navSection}>{n.label}</div>
      ...
      <div style={S.main}>
        ...
        {page==='banktxn'&&activeEntity&&<BankTransactions entityId={activeEntity} user={user}/>}
...
// ═══ Journal List (with attachments, memo on every row) ═══
function JournalList({entityId,onNewEntry}){
  const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);const[from,setFrom]=useState('');const[to,setTo]=useState('');
  ...
  const acctName=code=>accounts.find(a=>a.code===code)?.name||'?';
  ...
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    ...
    <button style={S.btnP} onClick={onNewEntry}>+ New Entry</button></div>
      ...
      entries.map(e=><div key={e.id} style={{borderBottom:'2px solid '+C.borderLight,paddingBottom:12,marginBottom:12}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <span style={{fontWeight:700,color:C.accent,fontSize:13}}>JE-{String(e.entry_num).padStart(4,'0')}</span>
            <span style={{color:C.textDim,fontSize:12}}>{e.date}</span>
            <span style={{color:C.text,fontSize:12}}>{e.memo}</span></div>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:11,color:C.textDim}}>{e.created_by}</span>
            <span style={{cursor:'pointer',color:C.red,fontSize:12}} onClick={()=>del(e.id)}>Delete</span></div></div>
        ...
        {e.attachments&&e.attachments.length>0&&<div style={{marginTop:6}}>{e.attachments.map(a=>
          <a key={a.id} href={api.downloadAttachment(a.id)} target="_blank" style={{...S.fileChip,textDecoration:'none',color:C.accent}}>{a.original_name} ({fmtSize(a.size)})</a>)}</div>}
      </div>)}</div></div>);}
...
function GeneralLedger({entityId}){const[entries,setEntries]=useState([]);const[accounts,setAccounts]=useState([]);const[filter,setFilter]=useState('');const[from,setFrom]=useState(fy_start());const[to,setTo]=useState(today());
  ...
  const doExport=()=>{const rows=[['General Ledger','','','','',''],['Period: '+(from||'Begin')+' to '+(to||today())],[]];
    filtered.forEach(acct=>{const txns=[];entries.forEach(e=>{e.lines.forEach(l=>{if(l.account_code===acct.code)txns.push({date:e.date,je:'JE-'+String(e.entry_num).padStart(4,'0'),memo:e.memo,debit:l.debit,credit:l.credit});});});
      if(txns.length===0&&!filter)return;rows.push([acctLabel(acct.code,acct.name)]);rows.push(['Date','JE','Memo','Debit','Credit','Balance']);
      let run=0;const isDr=acct.type==='Asset'||acct.type==='Expense';txns.sort((a,b)=>a.date.localeCompare(b.date)).forEach(t=>{run+=isDr?(t.debit-t.credit):(t.credit-t.debit);rows.push([t.date,t.je,t.memo,t.debit||'',t.credit||'',run]);});rows.push([]);});
    exportToExcel(rows,'GL.xlsx');};
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={S.h1}>General Ledger</div><button style={S.btnExport} onClick={doExport}>Export Excel</button></div><div style={S.sub}/>
    ...
    {filtered.map(acct=>{const txns=[];entries.forEach(e=>{e.lines.forEach(l=>{if(l.account_code===acct.code)txns.push({...l,date:e.date,memo:e.memo,jeNum:e.entry_num});});});
      ...
      return(<div key={acct.code} style={S.card}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}><span style={{fontWeight:700,color:C.textBright}}>{acct.code}</span><span>{acct.name}</span><span style={S.tag(acct.type)}>{acct.type}</span></div>
        ...
        <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>JE</th><th style={S.th}>Memo</th><th style={S.thR}>Debit</th><th style={S.thR}>Credit</th><th style={S.thR}>Balance</th></tr></thead>
          <tbody>{txns.map((t,i)=>{run+=dr?(t.debit-t.credit):(t.credit-t.debit);return<tr key={i}><td style={S.td}>{t.date}</td><td style={S.td}><span style={{color:C.accent}}>JE-{String(t.jeNum).padStart(4,'0')}</span></td><td style={S.td}>{t.memo}</td><td style={S.tdR}>{t.debit>0?fmt(t.debit):''}</td><td style={S.tdR}>{t.credit>0?fmt(t.credit):''}</td><td style={{...S.tdR,fontWeight:600}}>{fmt(run)}</td></tr>;})}</tbody></table>}</div>);})}</div>);}

// ═══ Bank Transactions (Upload + Code) ═══
function BankTransactions({entityId,user}){
  const[accounts,setAccounts]=useState([]);const[bankAccts,setBankAccts]=useState([]);const[txns,setTxns]=useState([]);
  const[selAcct,setSelAcct]=useState('');const[statusFilter,setStatusFilter]=useState('pending');const[err,setErr]=useState('');const[msg,setMsg]=useState('');
  ...
  const handleUpload=async(e)=>{const file=e.target.files[0];if(!file||!selAcct)return;e.target.value='';setErr('');setMsg('');
    try{const r=await api.uploadBankTransactions(entityId,selAcct,file);setMsg(r.count+' transactions imported');load();}catch(ex){setErr(ex.message);}};

  const codeTransaction=async(id,account_code,memo)=>{await api.codeBankTransaction(entityId,id,account_code,memo);load();};
  const postCoded=async()=>{const ids=txns.filter(t=>t.status==='coded').map(t=>t.id);if(ids.length===0){setErr('No coded transactions');return;}
    try{const r=await api.postBankTransactions(entityId,ids);setMsg(r.posted+' journal entries created');load();}catch(ex){setErr(ex.message);}};
  const delTxn=async id=>{await api.deleteBankTransaction(entityId,id);load();};
  ...
  return(<div><div style={S.h1}>Bank Transactions</div><div style={S.sub}>Upload bank statements, code transactions, and post to GL</div>
    <div style={S.card}><div style={S.h2}>Select Bank Account</div><div style={S.row}>
      ...
      {selAcct&&<div><input type="file" accept=".csv,.xlsx,.xls" id="bank-upload" style={{display:'none'}} onChange={handleUpload}/>
        <label htmlFor="bank-upload" style={{...S.btnP,display:'inline-block',cursor:'pointer',marginTop:14}}>Upload CSV/Excel</label></div>}
    ...
    {selAcct&&txns.length>0&&<div style={S.card}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        ...
        {txns.some(t=>t.status==='coded')&&<button style={S.btnP} onClick={postCoded}>Post {txns.filter(t=>t.status==='coded').length} Coded to GL</button>}</div>
      <table style={S.table}><thead><tr><th style={S.th}>Date</th><th style={S.th}>Description</th><th style={S.thR}>Amount</th><th style={S.th}>GL Account</th><th style={S.th}>Memo</th><th style={S.th}>Status</th><th style={{...S.th,width:40}}></th></tr></thead>
        <tbody>{txns.map(t=><tr key={t.id} style={t.status==='posted'?{opacity:0.5}:{}}>
          ...
          <td style={{...S.td,padding:'3px 4px'}}>{t.status==='posted'?<span style={{fontSize:11,color:C.textDim}}>{t.account_code}</span>:
            <select style={S.selectSm} value={t.account_code||''} onChange={e=>codeTransaction(t.id,e.target.value,t.memo)}>
              <option value="">- Code -</option>{accounts.filter(a=>a.code!==selAcct).sort((a,b)=>a.code.localeCompare(b.code)).map(a=><option key={a.code} value={a.code}>{acctLabel(a.code,a.name)}</option>)}</select>}</td>
          ...
          <td style={S.td}><span style={{fontSize:10,fontWeight:600,padding:'2px 6px',borderRadius:4,
            background:t.status==='posted'?C.green+'20':t.status==='coded'?C.accent+'20':C.orange+'20',
            color:t.status==='posted'?C.green:t.status==='coded'?C.accent:C.orange}}>{t.status}</span></td>
          <td style={S.td}>{t.status!=='posted'&&<span style={{cursor:'pointer',color:C.red,fontSize:11}} onClick={()=>delTxn(t.id)}>x</span>}</td>
        </tr>)}</tbody></table></div>}
...
function TrialBalance({entityId}){const[balances,setBalances]=useState([]);const[asOf,setAsOf]=useState(today());const fyStart=asOf.slice(0,4)+'-01-01';
  ...
  let tDr=0,tCr=0;const rows=balances.filter(b=>Math.abs(b.balance)>0.005).map(b=>{const isDr=b.type==='Asset'||b.type==='Expense';
    const dr=(isDr&&b.balance>0)||(!isDr&&b.balance<0)?Math.abs(b.balance):0;const cr=(isDr&&b.balance<0)||(!isDr&&b.balance>0)?Math.abs(b.balance):0;tDr+=dr;tCr+=cr;return{...b,dr,cr};});
...
function BankReconciliation({entityId,user}){const[accounts,setAccounts]=useState([]);const[entries,setEntries]=useState([]);const[recs,setRecs]=useState([]);
  ...
  if(view==='new')return(<div><button style={{...S.btnS,marginBottom:16}} onClick={()=>{setView('list');setSelAcct('');setChecked({});}}>Back</button>
    ...
    {selAcct&&<><div style={S.card}><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:10}}>
      ...
      <div style={S.card}><div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
        <div style={S.h2}>Uncleared ({uncl.length})</div><div style={{display:'flex',gap:6}}>
      ...
      <div style={{display:'flex',justifyContent:'flex-end',gap:10}}>{err&&<span style={S.err}>{err}</span>}<button style={isRec?S.btnP:{...S.btnP,opacity:.5,cursor:'not-allowed'}} onClick={finalize}>{isRec?'Finalize':'Diff must be $0'}</button></div></>}</div>);
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
...
function EntityManagement({refresh,entities,activeEntity,setActiveEntity}){const[showAdd,setShowAdd]=useState(false);const[bulk,setBulk]=useState(false);const[form,setForm]=useState({code:'',name:''});const[bulkText,setBulkText]=useState('');const[err,setErr]=useState('');
  return(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
    ...
    {bulk&&<div style={{...S.card,borderColor:C.accent+'60'}}><div style={S.h2}>Bulk Import</div><textarea style={{...S.input,height:140,fontFamily:'monospace',fontSize:11,resize:'vertical'}} placeholder="CODE, Entity Name" value={bulkText} onChange={e=>setBulkText(e.target.value)}/>
      {err&&<div style={S.err}>{err}</div>}<button style={{...S.btnP,marginTop:8}} onClick={async()=>{const ents=bulkText.split('\n').map(l=>l.trim()).filter(Boolean).map(l=>{const[c,...r]=l.split(',').map(p=>p.trim());return{code:c,name:r.join(',')};}).filter(e=>e.code&&e.name);
        if(!ents.length){setErr('None');return;}try{await api.bulkCreateEntities(ents);setBulkText('');setBulk(false);refresh();}catch(e){setErr(e.message);}}}>Import</button></div>}
