...
app.get('/api/entities/:eid/balances', auth, (req, res) => {
  ...
  if (close_pl_before && as_of) {
    ...
    const results = bsRows.map(r => { const isDr=r.type==='Asset'||r.type==='Expense'; let bal=isDr?(r.total_debit-r.total_credit):(r.total_credit-r.total_debit);
      if (r.account_code==='3100') bal+=priorNI;
    ...
    if (Math.abs(priorNI)>0.005 && !results.find(r=>r.code==='3100')) {
      const re=db.prepare('SELECT * FROM accounts WHERE entity_id=? AND code=?').get(req.params.eid,'3100');
      if (re) results.push({ code:'3100', name:re.name, type:re.type, subtype:re.subtype, bank_acct:0, balance:priorNI, total_debit:0, total_credit:0 });
    }
