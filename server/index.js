...
// Account type derived from code: <=19999 Asset, <=29999 Liability, <=39999 Equity, <=49999 Revenue, >=50000 Expense
app.post('/api/entities/:eid/import-tb', auth, requireRole('Admin','Accountant'), memUpload.single('file'), (req, res) => {
  ...
  try {
    ...
    const codeCol = findCol('account number', 'account #', 'acct', 'code', 'number');
    const nameCol = findCol('account name', 'name', 'description', 'account');
    const amtCol = findCol('amount', 'balance', 'total');
    const drCol = findCol('debit');
    const crCol = findCol('credit');
    ...
    for (const row of rows) {
      ...
      let signedAmt = 0;
      if (drCol || crCol) {
        const dr = parseFloat(String(row[drCol] || '0').replace(/[,$()]/g, '')) || 0;
        const cr = parseFloat(String(row[crCol] || '0').replace(/[,$()]/g, '')) || 0;
        // Signed amount: positive = natural balance side
        // Asset/Expense are debit-natural, others credit-natural
        if (type === 'Asset' || type === 'Expense') signedAmt = dr - cr;
        else signedAmt = cr - dr;
      } else if (amtCol) {
      ...
      parsed.push({ code, name, type, amount: signedAmt });
    }
    ...
    // For Asset/Expense (debit-natural): positive amount -> debit, negative -> credit
    // For Liability/Equity/Revenue (credit-natural): positive amount -> credit, negative -> debit
    const lines = [];
    ...
    for (const p of parsed) {
      ...
      let dr = 0, cr = 0;
      if (isDebitNatural) {
        if (p.amount >= 0) dr = p.amount; else cr = -p.amount;
      } else {
        if (p.amount >= 0) cr = p.amount; else dr = -p.amount;
      }
      lines.push({ account_code: p.code, debit: dr, credit: cr });
      totalDr += dr; totalCr += cr;
    }

    // Check if balanced; if not, add a plug to retained earnings
    const diff = +(totalDr - totalCr).toFixed(2);
    let plugAdded = false;
    if (Math.abs(diff) > 0.005) {
      // Find or create retained earnings (31000)
      const retainedCode = parsed.find(p => p.code === '31000') ? '31000' : null;
      if (retainedCode) {
        // Add to retained earnings line
        ...
        return res.status(400).json({ error: 'Trial balance does not balance (off by ' + diff.toFixed(2) + ') and no Retained Earnings (31000) account found to plug the difference.' });
      }
    }
...
app.post('/api/entities/:eid/bank-transactions/upload', auth, requireRole('Admin','Accountant'), memUpload.single('file'), (req, res) => {
  ...
  try {
    ...
    // Detect debit and credit columns FIRST (before generic amount), with broader keyword set
    ...
    // Only use generic "amount" column if we don't already have separate debit/credit columns
    ...
    db.transaction(() => {
      for (const row of rows) {
        ...
        let amount = 0;
        // Strategy 1: Both debit and credit columns - credits positive (inflows), debits negative (outflows)
        if (debitCol && creditCol) {
          const dr = Math.abs(parseNum(row[debitCol]));
          const cr = Math.abs(parseNum(row[creditCol]));
          amount = cr - dr;
        }
        // Strategy 2: Only one signed amount column
        else if (amountCol && row[amountCol] !== '' && row[amountCol] != null) {
          amount = parseNum(row[amountCol]);
        }
        // Strategy 3: Only debit OR only credit column (treat missing as 0)
        else if (debitCol) {
