  ...
  try {
    ...
    if (isPdf) {
      ...
      const parseAmt = s => {
        ...
        const isParen = /^\(.*\)$/.test(clean);
        let v = parseFloat(clean.replace(/[()]/g, '')) || 0;
        if (isParen) v = -v;
        if (s.includes('-') && v > 0) v = -v;
        return v;
      };

      // Try to identify if there's a running balance column (last number in a multi-number line)
      // Heuristic: lines with dates that have 2+ amounts — last one is balance, prior one(s) are the txn
      let lastDate = null;
      for (const line of lines) {
        ...
        // Only process lines that start with a date OR are a continuation of a dated line
        ...
        if (amounts.length === 1) {
          ...
          // First is the transaction amount, second is running balance — take the first
          amount = parseAmt(amounts[0]);
        } else if (amounts.length >= 3) {
          // Likely: debit, credit, balance — or amount, balance, something else
          // Take the first non-zero as the amount
