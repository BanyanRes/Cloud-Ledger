...
const UPLOAD_DIR = path.join(path.dirname(DB_PATH), 'attachments');

const dataDir = path.dirname(DB_PATH);
if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });

const ALLOWED_ATTACHMENT_EXTS = new Set(['pdf','xlsx','xls','csv','png','jpg','jpeg','eml','msg','doc','docx']);
...
app.post('/api/entities/:eid/entries/:id/attachments', auth, requireRole('Admin','Accountant'), upload.array('files', 10), (req, res) => {
  const entry = db.prepare('SELECT id FROM journal_entries WHERE id=? AND entity_id=?').get(req.params.id, req.params.eid);
  if (!entry) return res.status(404).json({ error: 'Journal entry not found' });
  if (!req.files || req.files.length === 0) return res.status(400).json({ error: 'No files' });
  const invalid = req.files.filter(f => !ALLOWED_ATTACHMENT_EXTS.has(getExt(f.originalname)));
  if (invalid.length) return res.status(400).json({ error: 'Unsupported attachment type' });
