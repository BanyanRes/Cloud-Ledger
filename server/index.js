...
const { buildApAging } = require('./apaging');
...
app.get('/api/billcom/ap-aging/:entity_id', auth, requireEntityAccess('entity_id'), requireRole('Admin', 'Accountant'), async (req, res) => {
  ...
  res.json(buildApAging(db, entityId, asOf));
});
...
// Workpapers › Quarterly Closing by year and quarter.
require('./quarterlyclose').registerQuarterlyCloseRoutes(app, {
  db,
  auth,
  requireEntityAccess,
  requireRole,
...
// ═══ Month-End Leadsheets (monthly FloQast-format close package) ═══
// One workbook per leadsheet (Cash, AR, Prepaid, Other Assets, Fixed Assets,
// AP, Accrued, Other Liabilities, Debt, Intercompany, Equity), built from the
// GL in the entity's FloQast layout. Filed under Workpapers › Month-End
// Leadsheets by year and month; downloaded as one .zip. See server/leadsheets.js.
require('./leadsheets').registerLeadsheetRoutes(app, {
  db,
  auth,
  requireEntityAccess,
  requireRole,
  workpapersDir: WORKPAPERS_DIR,
  computeBalances: (eid, opts) => computeBalances(eid, opts),
  apAging: (eid, asOf, acct) => buildApAging(db, eid, asOf, acct),
});
  ...
  try {
    ...
    if (pcapData) {
      ...
      try {
        ...
        const fees = gpfees.weaverSchedule(gpfees.buildData(ctx, gpfees.resolveQuarter(asOf)));
        supp.push({ kind: 'fees', tocLabel: 'Schedule of Fees Paid To The General Partners and Affiliates', data: fees });
      ...
      try {
        ...
        const cdata = carry.buildData(ctx, carry.resolveQuarter(asOf), { entity_id: Number(eid) });
        supp.push({ kind: 'carry', tocLabel: 'Carried Interest Reporting per Section §17(c) of GCM Grosvenor Side Letter Agreement', data: cdata });
