...
// in the GL, so we skip anything dated on/before it. The sync engine compares
// the cutoff against each bill's GL POSTING date (not its invoice date) for
// every entity except CLRF and Turnkey Rail, and skips when
// postingDate <= cutoff — so the cutoff date itself is excluded, and the caller
// stores latestBillDate + 1 day only to keep the stored value aligned with the
// aging report it came from. Creating a fresh config row here would be missing
// required credentials, so this only updates an existing config.
...
// TEMP read-only audit (remove after use): for each synced bill, compare the
// Bill.com department's mapped project against what is actually stored on the CL
// debit lines. Writes nothing. Batched like retag-projects.
// Body: { limit?, offset?, target_project? }. If target_project is set, only bills
// touching that project (expected OR stored) are returned.
app.post('/api/billcom/_coding-audit/:entity_id', auth, requireEntityAccess('entity_id'), requireRole('Admin', 'Accountant'), async (req, res) => {
 try {
  const pick = (obj, ...keys) => { for (const k of keys) if (obj && obj[k] != null) return obj[k]; return null; };
  const norm = (v) => { if (v == null || v === '') return null; const s = String(v); const m = /^(\d+)\.0+$/.exec(s); return m ? m[1] : s; };
  const eid = parseInt(req.params.entity_id);
  const target = (req.body && req.body.target_project != null) ? norm(req.body.target_project) : null;
  const limit = Math.max(1, Math.min(parseInt((req.body && req.body.limit) || 20, 10) || 20, 100));
  const offset = Math.max(0, parseInt((req.body && req.body.offset) || 0, 10) || 0);
  const cfg = db.prepare('SELECT * FROM billcom_config WHERE entity_id = ?').get(eid);
  if (!cfg) return res.status(400).json({ error: 'Bill.com not configured for this entity' });
  const projMap = new Map(db.prepare('SELECT billcom_dept_id, cl_project_id FROM billcom_project_map WHERE entity_id = ?').all(eid).map(r => [String(r.billcom_dept_id), norm(r.cl_project_id)]));
  const deptName = new Map(db.prepare('SELECT billcom_dept_id, billcom_dept_name FROM billcom_project_map WHERE entity_id = ?').all(eid).map(r => [String(r.billcom_dept_id), r.billcom_dept_name]));
  const allSynced = db.prepare(
    "SELECT bl.billcom_id, bl.cl_entry_id, bl.invoice_number, je.date, je.entry_num, je.doc_number, je.vendor " +
    "FROM billcom_sync_log bl JOIN journal_entries je ON je.id = bl.cl_entry_id " +
  ...
  const synced = allSynced.slice(offset, offset + limit);
  const nextOffset = offset + synced.length;
  const result = { total, offset, examined: synced.length, next_offset: nextOffset < total ? nextOffset : null, remaining: Math.max(0, total - nextOffset), ok: 0, blank: 0, mismatch: 0, no_map: 0, errors: [], bills: [] };
  if (!synced.length) return res.json(result);
  ...
  const args = { sessionId: session.sessionId, devKey, baseUrl: cfg.api_base_url };
  const lineQ = db.prepare('SELECT id, account_code, debit, credit, project_id, description FROM journal_lines WHERE entry_id = ? ORDER BY id');

  for (const row of synced) {
    let detail;
    try { detail = await billcomGetById({ ...args, resourcePath: '/bills', id: row.billcom_id }); }
    catch (e) { result.errors.push({ billcom_id: row.billcom_id, entry_num: row.entry_num, error: e.message }); continue; }
    const lineItems = pick(detail, 'lineItems', 'line_items', 'billLineItems') || [];
    const clLines = lineQ.all(row.cl_entry_id).filter(l => (l.debit || 0) > 0);
    const lines = [];
    let worst = 'ok';
    lineItems.forEach((li, i) => {
      const cls = pick(li, 'classifications') || {};
      const deptId = String(pick(cls, 'departmentId', 'deptId', 'department') || pick(li, 'departmentId', 'deptId') || '');
      const expected = deptId ? (projMap.get(deptId) || null) : null;
      const clLine = clLines[i];
      const stored = clLine ? norm(clLine.project_id) : null;
      let status;
      if (!deptId) status = 'no_dept';
      else if (expected == null) status = 'no_map';
      else if (stored == null) status = 'blank';
      else if (stored === expected) status = 'ok';
      else status = 'mismatch';
      if (status === 'mismatch') worst = 'mismatch';
      else if (status === 'blank' && worst !== 'mismatch') worst = 'blank';
      else if ((status === 'no_map' || status === 'no_dept') && worst === 'ok') worst = status;
      lines.push({ dept: deptName.get(deptId) || deptId || null, expected, stored, status });
    });
    if (worst === 'ok') result.ok++;
    else if (worst === 'mismatch') result.mismatch++;
    else if (worst === 'blank') result.blank++;
    else result.no_map++;
    if (target != null) {
      const touches = lines.some(l => l.expected === target || l.stored === target);
      if (!touches) continue;
    }
    result.bills.push({ entry_num: row.entry_num, date: row.date, doc_number: row.doc_number, vendor: row.vendor, billcom_id: row.billcom_id, worst, lines });
  }
  res.json(result);
 } catch (e) {
  console.error('coding-audit failed: ' + (e && e.stack || e));
  if (!res.headersSent) res.status(500).json({ error: e.message });
 }
});
