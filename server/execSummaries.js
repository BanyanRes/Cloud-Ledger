...
function resolveDateLine(mode, meta) {
  ...
  // Fallback so the date line is never undefined: statements.meta.monthsEnded is
  // always populated on the real path, but a caller (or a stored-text override
  // whose dateMode resolves to 'monthsEnded') might not set it. Derive a plain
  // "For the Period Ended <asOf>" label from asOf in that case.
  const monthsEnded = meta.monthsEnded || ('For the Period Ended ' + longDate(asOf));
  if (!mode || mode === 'monthsEnded') return monthsEnded;
...
async function renderExecSummaryPdf(meta, override) {
  const builtIn = resolveSummary(meta);
  // Merge a stored override (blocks required; title/dateMode optional) over
  // the built-in def. Either source alone is enough to render.
  let def = builtIn;
  if (override && Array.isArray(override.blocks) && override.blocks.length) {
    def = {
      title: override.title || (builtIn && builtIn.title) || (meta.entityName || ''),
      dateMode: override.dateMode || (builtIn && builtIn.dateMode) || 'monthsEnded',
...
// ── Parse an uploaded exec-summary PDF into renderer blocks ──────────────────
// CLA authors these in their own tool and upload a flattened PDF whose printed
// date never updates. We extract the BODY text once, on upload, and store it as
// { p } / { bullets } blocks; the renderer then redraws it every month with a
// DYNAMIC date line (drawn from the entity's dateMode + the run's asOf), so the
// only thing that changes month to month is the date. Title and date line are
// NOT taken from the PDF — they are dropped here and regenerated by the renderer,
// which is what makes the date dynamic.
//
// Reconstructs lines from pdfjs text items by Y position (a single space-joined
// blob loses the paragraph/bullet structure), then:
//   • drops the 3-line title block (entity / "Executive Summary" / date line),
//   • drops a stray page-number-only line and the "Notes to the Reader" header,
//   • groups the rest into paragraphs, treating any line beginning with a bullet
//     glyph (•, ·, -, *, or the mojibake �) as a new bullet, and joining wrapped
//     continuation lines into the block they belong to.
// Returns { blocks } (array) or null if nothing usable was found.
async function parseExecSummaryBlocks(pdfBuffer) {
  const pdfjs = require('pdfjs-dist/legacy/build/pdf.js');
  const doc = await pdfjs.getDocument({ data: new Uint8Array(pdfBuffer), isEvalSupported: false, verbosity: 0 }).promise;

  // Gather lines across all pages, in reading order. Group text items whose
  // baselines are within a small Y tolerance into one line.
  const lines = [];
  ...
  // Drop everything up to and including the title block. The title block is the
  // entity line, the "Executive Summary" line, and the "For the ... " date line.
  // Find the date line ("For the ... 20xx") and start body after it; also drop a
  // leading page-number-only line if present.
  let startIdx = 0;
  const dateLineIdx = raw.findIndex(s => /^For the .*\b20\d{2}\b/i.test(s));
  if (dateLineIdx >= 0) startIdx = dateLineIdx + 1;
  else {
    // No recognizable date line: fall back to dropping an "Executive Summary"
    // line and anything above it.
  ...
  // Group into blocks, rejoining wrapped lines. The hard part is that a bullet
  // often wraps onto one or more lines that do NOT start with a bullet glyph, so
  // a naive "non-bullet line = new paragraph" split shatters bullets. Rule:
  //   \u2022 A bullet glyph line opens a new bullet item within the current run.
  //   \u2022 While a bullet run is OPEN, a non-bullet line CONTINUES the current bullet
  //     until that bullet text ends in sentence-final punctuation (. : ; !). The
  //     first line after a completed bullet that is itself non-bullet closes the
  //     run and becomes a paragraph.
  ...
  for (const line of body) {
    if (isBulletStart(line)) {
      ...
      curBullets.push(stripBullet(line));
      bulletOpen = !endsSentence(curBullets[curBullets.length - 1]);
    } else if (curBullets && bulletOpen) {
      // Continuation of the current wrapped bullet.
      const i = curBullets.length - 1;
      curBullets[i] = (curBullets[i] + ' ' + line).trim();
